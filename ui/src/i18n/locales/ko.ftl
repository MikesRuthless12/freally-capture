...
sources-add-background-music = 배경 음악…
sources-background-music-note = 반복 재생되며 미리보기에서 들리고 녹화/스트림에 포함됩니다. 영상 소리 대신 음악을 쓰려면 믹서에서 영상의 Backdrop 스트립을 음소거하세요.
sources-background-music-count = { $n }곡
sources-background-music-shuffle = 셔플
...
sources-stream-hide = 방송에서 숨기기
sources-stream-show = 방송에서 표시
sources-stream-hide-item = { $name } 방송에서 숨기기
sources-stream-show-item = { $name } 방송에서 표시
sources-record-hide = 녹화에서 숨기기
sources-record-show = 녹화에서 표시
sources-record-hide-item = { $name } 녹화에서 숨기기
sources-record-show-item = { $name } 녹화에서 표시
...
stats-rehearsal-note = 리허설 — 대상은 로컬 싱크에만 게시합니다
stats-timeline-open = 타임라인
timeline-title = 세션 타임라인
timeline-empty = 아직 기록이 없습니다 — 스트리밍이나 녹화 중에 타임라인이 기록됩니다.
timeline-live = LIVE — 아직 기록 중
timeline-fit = 맞춤
timeline-legend-fps = fps
timeline-legend-behind = 인코더 대기열(지연 프레임)
...
scenes-recent-title = 최근
scenes-recent-clear = 지우기
scenes-recent-clear-confirm = 최근 장면 목록을 지울까요?
...
livebutton-rehearse = 리허설
livebutton-rehearse-record-confirm = 이 리허설도 동영상 파일로 녹화할까요?
livebutton-rehearse-title = 전체 쇼를 로컬 싱크로 실행 — 아무것도 전송되지 않음
livebutton-end-rehearsal = 리허설 종료
livebutton-title-rehearsing = 리허설 진행 중 — 이 컴퓨터 밖으로 나가는 것이 없습니다
livebutton-badge-rehearsal = 리허설
livebutton-aria-rehearsal = 리허설 중
livebutton-rehearsal-banner = 리허설 — 이 컴퓨터 밖으로 나가는 것이 없습니다
...
properties-timer-clock = 특정 시각까지 카운트다운
...
filters-name-perspective = 원근
filters-name-fade-loop = 페이드 루프
...
filters-tilt = 기울기
filters-far-fade = 먼 가장자리 페이드
filters-fade-in-s = 페이드 인(초)
filters-visible-s = 표시(초)
filters-fade-out-s = 페이드 아웃(초)
filters-hidden-s = 숨김(초)
...
bench-open = 인코더 벤치마크 실행…
bench-title = 인코더 벤치마크
bench-intro = 이 컴퓨터에서 짧은 실측 인코딩 사다리(감지된 모든 인코더 × 프리셋 × 해상도)를 실행합니다 — 약 1분, 완전 오프라인, 아무것도 밖으로 나가지 않습니다. 실패는 숨기지 않고 표시됩니다. 먼저 방송·녹화를 중지하세요.
bench-start = 벤치마크 시작
bench-rerun = 다시 실행
bench-running = 측정 중… { $done } / { $total }
bench-cancel = 취소
bench-col-encoder = 인코더
bench-col-preset = 프리셋
bench-col-rung = 단계
bench-col-achieved = fps
bench-col-headroom = 여유
bench-failed = 실패
bench-rec-title = 권장(실측)
bench-rec-body = { $encoder } ({ $preset }), { $width }×{ $height } @ { $fps } fps — 실측 { $headroom }× 실시간. 권장 방송 비트레이트: { $bitrate } kbps.
bench-rec-none = 이 컴퓨터에서는 어떤 설정도 실시간을 유지하지 못합니다 — 캔버스 해상도나 fps를 낮추고 다시 측정하세요.
bench-apply = 녹화 설정에 적용
bench-applied = 적용됨 ✓
...
stream-session-report = 세션 종료 시 녹화 옆에 세션 보고서(HTML + Markdown)를 기록
stream-simulator-title = 네트워크 시뮬레이터(리허설)
stream-simulator-note = 리허설의 로컬 싱크만 제한 — 재연결과 약한 업링크를 훈련. 실제 방송은 절대 저하되지 않습니다.
stream-simulator-profile = 프로필
stream-simulator-off = 끔
stream-simulator-hotel-wifi = 호텔 Wi-Fi
stream-simulator-mobile-hotspot = 모바일 핫스팟
stream-simulator-custom = 사용자 지정
stream-simulator-bandwidth = 대역폭(kbps, 0 = 무제한)
stream-simulator-latency = 지연(ms)
stream-simulator-jitter = 지터(± ms)
stream-simulator-outage-every = 끊김 주기(초, 0 = 없음)
stream-simulator-outage-len = 끊김 길이(초)
...
workspace-pack-title = 장면 모음 팩
workspace-pack-export = 팩 내보내기…
workspace-pack-exporting = 내보내는 중…
workspace-pack-import = 팩 가져오기…
workspace-pack-importing = 가져오는 중…
workspace-pack-hint = .fcappack은 활성 모음과 로컬 에셋을 하나의 이동 가능한 파일로 묶습니다 — 서버 없이 전체 레이아웃을 공유하세요. 가져오기는 이를 새 모음으로 불러와 전환합니다.
workspace-pack-exported = "{ $name }" 내보냄 — 에셋 { $bundled }개 묶음, { $external }개 외부.
workspace-pack-imported = "{ $name }" 가져옴 — 장면 { $scenes }개, 에셋 { $relinked }개 재연결, { $external }개 외부.
workspace-compare = 비교 및 병합…
compare-title = 비교 및 병합
compare-intro = 활성 모음을 다른 모음과 비교하고 선택한 변경 사항만 가져옵니다. 아무것도 이 컴퓨터를 떠나지 않습니다.
compare-target-label = 비교 대상
compare-target-none = 모음 선택…
compare-target-pack = 팩 파일에서…
compare-against = { $name }와(과) 비교
compare-comparing = 비교 중…
compare-no-diff = 차이 없음 — 모음이 일치합니다.
compare-select-all = 모두 선택
compare-clear = 지우기
compare-merge-hint = 장면을 선택하면 사용하는 소스도 함께 가져옵니다.
compare-sources-title = 소스
compare-scenes-title = 장면
compare-kind-added = 추가됨
compare-kind-removed = 제거됨
compare-kind-modified = 변경됨
compare-renamed = 이전 이름 "{ $from }"
compare-reordered = 순서 변경됨
compare-apply = { $count }개 적용
compare-apply-none = 적용할 변경 사항 선택
compare-applying = 적용 중…
compare-aspect-source = 소스
compare-aspect-transform = 위치
compare-aspect-visibility = 표시 여부
compare-aspect-outputs = 출력
compare-aspect-filters = 필터
compare-aspect-lock = 잠금
compare-aspect-blend = 블렌드
compare-aspect-scaling = 스케일링
compare-aspect-backdrop = 배경
compare-aspect-reveal = 드러내기
compare-aspect-name = 이름
compare-aspect-settings = 설정
compare-aspect-audio = 오디오
workspace-snapshots = 스냅샷…
snapshots-title = 스냅샷
snapshots-intro = 돌아갈 수 있는 이 모음의 이름 있는 체크포인트 — 자동 저장과 실행 취소를 넘어 의도적으로 보관됩니다. 로컬에 저장됩니다.
snapshots-name-placeholder = 이 체크포인트 이름 지정…
snapshots-create = 스냅샷 저장
snapshots-empty = 아직 스냅샷이 없습니다 — 하나 저장해 돌아갈 수 있는 이름 있는 체크포인트를 남기세요.
snapshots-usage = { $cap }개 중 { $count }개 보관 · { $size }
snapshots-scenes = 장면 { $scenes }개 · 소스 { $sources }개
snapshots-restore = 복원
snapshots-compare = 비교
snapshots-delete = 스냅샷 삭제
snapshots-delete-aria = 스냅샷 "{ $name }" 삭제
snapshots-restore-confirm = 이 스냅샷을 복원할까요? 현재 레이아웃이 먼저 스냅샷으로 저장됩니다.

# --- Phase 8 (backup / theme editor / dock presets / settings search / quick actions) ---
workspace-backup = 백업 및 복원…
backup-title = 백업 및 복원
backup-intro = 설정, 프로필, 장면 컬렉션 등 스튜디오 전체를 하나의 파일로 저장하고, 여기서 또는 새 컴퓨터에서 복원하세요.
backup-export = 백업 내보내기…
backup-exporting = 내보내는 중…
backup-exported = 컬렉션 { $collections }개와 프로필 { $profiles }개를 백업했습니다 ({ $size }).
backup-restore = 백업에서 복원…
backup-secrets-note = 스트림 키와 비밀번호는 백업에 포함되지 않습니다 — 복원 후 다시 입력하세요.
backup-restore-from = { $created }에 만든 백업 (v{ $version })
backup-restore-settings = 설정
backup-restore-collections = 장면 컬렉션 ({ $count })
backup-restore-profiles = 프로필 ({ $count })
backup-restore-apply = 선택 항목 복원
backup-restoring = 복원하는 중…
backup-restored = 복원되었습니다.
backup-restart-note = 복원된 컬렉션과 프로필을 불러오려면 Freally Capture를 다시 시작하세요.
backup-cancel = 취소
theme-editor-title = 사용자 지정 팔레트
theme-color-bg = 배경
theme-color-panel = 패널
theme-color-text = 텍스트
theme-color-muted = 흐린 텍스트
theme-color-accent = 강조
theme-color-accent2 = 강조 2
theme-contrast-text-bg = 배경 위 텍스트
theme-contrast-text-panel = 패널 위 텍스트
theme-contrast-muted-bg = 배경 위 흐린 텍스트
theme-contrast-accent-bg = 배경 위 강조
theme-contrast-low = (권장 ≥ { $min }:1)
theme-reset = 어두운 테마로 재설정
theme-export = .fctheme 내보내기…
theme-import = .fctheme 가져오기…
workspace-dock-presets = 독 레이아웃
workspace-dock-preset-placeholder = 현재 레이아웃을 다른 이름으로 저장…
workspace-dock-preset-save = 저장
workspace-dock-preset-apply = “{ $name }”(으)로 전환
workspace-dock-preset-delete = 레이아웃 “{ $name }” 삭제
workspace-dock-presets-hint = 레이아웃은 통계 독과 믹서 방향을 기억합니다. 항목을 클릭하면 해당 레이아웃으로 전환됩니다.
settings-search-placeholder = 설정 검색…
settings-search-none = 일치하는 설정이 없습니다.
settings-changed = 열람 후 변경됨
menu-tools-quick-actions = 빠른 작업…
menu-tools-featured-chat = 주요 채팅 메시지…
quick-actions-title = 빠른 작업
quick-actions-empty = 아직 버튼이 없습니다 — 편집을 클릭해 추가하세요.
quick-actions-edit = 편집
quick-actions-done = 완료
quick-actions-add-page = 페이지
quick-actions-label = 버튼 레이블
quick-actions-color = 버튼 색상
quick-actions-add = 버튼 추가
quick-actions-remove = “{ $name }” 제거
quick-actions-delete-page = “{ $name }” 삭제
quick-actions-hint = 버튼은 단축키 및 LAN 패널과 동일한 스튜디오 명령을 실행합니다 — 장면, 매크로, 사운드보드 패드도 포함됩니다. 명령 버튼은 LAN 패널에도 표시됩니다.
quick-actions-cmd-transition = 전환
quick-actions-cmd-startStream = 라이브 시작
quick-actions-cmd-stopStream = 스트림 종료
quick-actions-cmd-startRecording = 녹화 시작
quick-actions-cmd-stopRecording = 녹화 중지
quick-actions-cmd-pauseRecording = 녹화 일시정지
quick-actions-cmd-addMarker = 마커 추가
quick-actions-cmd-armReplay = 리플레이 준비
quick-actions-cmd-saveReplay = 리플레이 저장
quick-actions-cmd-setStudioMode = 스튜디오 모드 전환
quick-actions-cmd-scene = 장면으로 전환…
quick-actions-cmd-macro = 매크로 실행…
quick-actions-cmd-soundboard = 사운드보드 패드…
quick-actions-value-scene = 장면 이름
quick-actions-value-macro = 매크로 이름
quick-actions-value-soundboard = 패드 ID
teleprompter-stop = 정지
about-portable-on = 포터블 모드: 켜짐 — 모든 앱 데이터가 앱 옆에 저장됩니다.
about-portable-off = 포터블 모드: 꺼짐. 앱을 완전히 독립적으로 실행하려면 앱 옆에 "{ $marker }" 파일을 추가하세요(USB 메모리 모드).
about-portable-data = 데이터 폴더: { $path }
...
history-toggleOutputVisibility = 출력별 표시 전환
...
palette-featured-chat = 주요 채팅 메시지…
featured-chat-title = 주요 채팅 메시지
featured-chat-intro = 최근 채팅 한 줄을 프로그램 하단 배너로 고정합니다. 스트림과 녹화 시청자에게 표시됩니다. 고정은 재시작 시 해제되고 색상은 유지됩니다.
featured-chat-empty = 아직 채팅이 없습니다. 채팅 오버레이 소스를 추가하고 채널에 연결하세요. 도착하는 대로 여기에 표시됩니다.
featured-chat-pinned = 프로그램 표시 중:
featured-chat-pin = 고정
featured-chat-clear = 해제
featured-chat-bg = 배너 색
featured-chat-text = 글자 색
featured-chat-note = 배너는 모든 장면 위, 하단 중앙에 표시됩니다. 해제하면 방송에서 사라집니다.
health-title = 소스 상태
...
sources-visualizer-classic = 클래식 색상
...
menu-collection-clear = 기록 지우기…
menu-collection-clear-confirm = 이 장면 컬렉션과 파일을 영구적으로 삭제할까요? { $names } — 활성 컬렉션 "{ $active }"은(는) 유지됩니다. 이 작업은 취소할 수 없습니다.
...
# --- Telestrator (CAP-N57) ---
telestrator-group = 텔레스트레이터
telestrator-draw = 그리기
telestrator-tool-pen = 펜
telestrator-tool-highlight = 형광펜
telestrator-tool-arrow = 화살표
telestrator-tool-ellipse = 타원
telestrator-color = 색상
telestrator-width = 굵기
telestrator-whiteboard = 화이트보드 모드
telestrator-persist = 유지
telestrator-fade = 페이드
telestrator-undo = 마지막 표시 실행 취소
telestrator-clear = 표시 지우기
hotkeys-telestrator-clear = 텔레스트레이터: 표시 지우기

# --- Teleprompter (CAP-N58) ---
teleprompter-title = 텔레프롬프터
teleprompter-loading = 불러오는 중…
teleprompter-empty = 아직 대본이 없습니다 — 텔레프롬프터 패널에서 추가하세요.
teleprompter-script = 대본
teleprompter-script-placeholder = 대본을 입력하거나 붙여넣으세요…
teleprompter-top = 맨 위
teleprompter-slower = 느리게
teleprompter-play = 재생
teleprompter-pause = 일시정지
teleprompter-faster = 빠르게
teleprompter-step-back = 뒤로
teleprompter-step-forward = 앞으로
teleprompter-est-time = 읽는 시간
teleprompter-caesura-hint = 일시정지하려면 -- 입력
teleprompter-caesura-pause = 일시정지 — 일시정지하려면 -- 입력
teleprompter-countdown = 카운트다운 시작
teleprompter-countdown-secs = 카운트다운 (초)
teleprompter-speed = 속도 (문자/초)
teleprompter-speed-bpm = 속도 (BPM)
teleprompter-bpm-mode = BPM 모드 (노래)
teleprompter-font = 글꼴 크기
teleprompter-mirror = 미러 (유리)
teleprompter-open-projector = 프로젝터 열기
teleprompter-preview = 미리보기
teleprompter-remote-hint = 이 창이 닫혀 있는 동안 단축키, MIDI 또는 LAN 패널로 스크롤을 제어하세요.
teleprompter-read-aloud = OS 기본 음성 합성으로 읽어주기
menu-tools-teleprompter = 텔레프롬프터
hotkeys-teleprompter-toggle = 텔레프롬프터: 재생 / 일시정지

# --- Remote guests: green room / cues / QoS / auto-grid (CAP-N54–N56, N59) ---
remote-hosting-count = 게스트 { $count }명 호스팅 중
remote-green-room-default = 그린룸
remote-green-room-default-title = 새 게스트는 앉힐 때까지 그린룸에서 대기합니다
remote-auto-grid = 자동 그리드
remote-auto-grid-title = 게스트가 들어오고 나갈 때 그리드로 유지
remote-arrange-grid = 그리드 배치
remote-arrange-grid-title = 방송 중인 게스트를 지금 그리드로 재배치
remote-green-room-monitor = 그린룸 미리보기
remote-tech-cam-ok = 카메라 ✓
remote-tech-cam-no = 카메라 없음
remote-tech-mic-ok = 마이크 ✓
remote-tech-mic-no = 마이크 없음
remote-seat-on-air = 온에어에 앉히기
remote-cues-label = 큐
remote-cue = 큐
remote-cue-thirty = 30초 남음
remote-cue-wrap = 마무리하세요
remote-cue-next = 다음은 당신입니다
remote-cue-speak = 더 크게 말하세요
remote-qos-good = 연결 양호
remote-qos-fair = 연결 보통
remote-qos-poor = 연결 불량
remote-green-room-guest = 그린룸에 계십니다 — 호스트가 곧 온에어로 올려드립니다.

# --- Hotkey audit — Phase 7 actions ---
hotkey-audit-action-telestrator-clear = 텔레스트레이터: 표시 지우기
hotkey-audit-action-teleprompter-toggle = 텔레프롬프터: 재생 / 일시정지
hotkey-audit-feature-telestrator = 텔레스트레이터
hotkey-audit-feature-teleprompter = 텔레프롬프터

# V1-C — Starting Soon countdown slate
sources-add-starting-soon = 곧 시작합니다…
sources-starting-soon-title = 곧 시작합니다
sources-starting-soon-default = 곧 시작합니다
sources-starting-soon-message = 메시지
sources-starting-soon-minutes = 분
sources-starting-soon-hours = 시간
sources-starting-soon-start-at = 시작 시각
sources-starting-soon-music = 배경 음악…
sources-starting-soon-music-name = 배경 음악
sources-starting-soon-background = 배경
sources-starting-soon-note = 전체 화면 카운트다운으로, 0에서 빨간색으로 깜박이며 멈춥니다. 준비되면 직접 지우고 라이브 장면으로 전환하세요.
sources-starting-soon-add = 추가
sources-slate-solid = 단색
sources-slate-gradient = 그라데이션
sources-slate-transparent = 투명
sources-slate-image = 이미지
sources-slate-file = 이미지 / 동영상 / GIF
sources-slate-browse = 찾아보기…
sources-starting-soon-choose-image = 이미지 선택…
sources-slate-media-note = 정지 이미지는 여기에 포함됩니다. 반복 동영상이나 GIF 배경은 '투명'을 선택하고 장면의 배경(장면 레일의 🖼)을 설정하세요 — 카운트다운이 그 위에 표시됩니다.
properties-timer-slate = '곧 시작합니다' 슬레이트
properties-timer-slate-off = 끔(인라인 텍스트)
properties-timer-message = 메시지
properties-timer-slate-color = 슬레이트 색상
properties-timer-slate-from = 시작
properties-timer-slate-to = 끝
properties-timer-slate-note = 투명으로 두면 슬레이트 아래의 이미지·동영상·장면 배경이 비쳐 보입니다.
