...
sources-add-background-music = 背景音乐…
sources-background-music-note = 循环播放，可在预览中听到并录制/推流。若想用音乐代替视频声音，请在混音器中静音视频的 Backdrop 轨道。
sources-background-music-count = { $n } 首
sources-background-music-shuffle = 随机
...
sources-stream-hide = 在直播中隐藏
sources-stream-show = 在直播中显示
sources-stream-hide-item = 在直播中隐藏 { $name }
sources-stream-show-item = 在直播中显示 { $name }
sources-record-hide = 在录制中隐藏
sources-record-show = 在录制中显示
sources-record-hide-item = 在录制中隐藏 { $name }
sources-record-show-item = 在录制中显示 { $name }
...
controls-marker = ◈ 标记
controls-iso-lanes = 与节目并行录制的 ISO 通道：{ $count }
...
ltc-badge = LTC
ltc-title = SMPTE 时间码（LTC）
ltc-intro = 在某条轨道上生成 SMPTE 线性时间码，并从任意音频输入读取传入的 LTC — 用于后期同步外部录音机和摄像机的经典音频时间码。完全离线。
ltc-generate = 在轨道上生成 LTC
ltc-track = 时间码轨道
ltc-track-option = 轨道 { $track }
ltc-fps = 帧率
ltc-read = 读取 LTC 来源
ltc-read-off = 关闭
ltc-decoded = 传入时间码
ltc-no-lock = 无信号
ltc-note = 生成器同步到当日时间，非丢帧。录制其轨道（在输出设置中指定），或路由到某个输出以馈送外部设备。读取器驱动统计叠加层的时间码行，并给章节标记盖上时间码。
...
stats-disk = 磁盘
stats-disk-free = 可用
stats-disk-left = 剩余录制
stats-disk-rate = ≈ { $rate } MB/s 录制
stats-vertical-fps = 9:16 FPS
stats-targets-label = 推流目标
stats-rehearsal-note = 彩排 — 目标仅发布到本地接收端
stats-timeline-open = 时间线
timeline-title = 会话时间线
timeline-empty = 尚无记录 — 直播或录制期间会记录时间线。
timeline-live = 直播中 — 仍在记录
timeline-fit = 适应
timeline-legend-fps = fps
timeline-legend-behind = 编码器队列（落后帧数）
...
scenes-recent-clear = 清除
scenes-recent-clear-confirm = 清除最近场景列表？
...
livebutton-rehearse = 彩排
livebutton-rehearse-record-confirm = 同时将这次排练录制为视频文件？
livebutton-rehearse-title = 将整场直播跑在本地接收端上 — 不发送任何数据
livebutton-end-rehearsal = 结束彩排
livebutton-title-rehearsing = 彩排进行中 — 没有任何数据离开这台电脑
livebutton-badge-rehearsal = 彩排
livebutton-aria-rehearsal = 彩排中
livebutton-rehearsal-banner = 彩排 — 没有任何数据离开这台电脑
...
properties-timer-clock = 倒计时到某个时间
...
filters-name-perspective = 透视
filters-name-fade-loop = 淡入淡出循环
...
filters-tilt = 倾斜
filters-far-fade = 远端淡出
filters-fade-in-s = 淡入（秒）
filters-visible-s = 显示（秒）
filters-fade-out-s = 淡出（秒）
filters-hidden-s = 隐藏（秒）
filters-crop-left = 左
filters-crop-top = 上
...
recordings-trim = 修剪
recordings-trim-title = 从此录像中剪出片段 — 对齐关键帧的剪切无需重新编码即可导出
recordings-verify = 校验
recordings-verify-title = 检查文件完整性 — 容器结构、连续性、音视频交织、时长
recordings-verifying = 正在校验…
verify-dismiss = 关闭
verify-verdict-pass = { $name } — 完整性正常
verify-verdict-warn = { $name } — 校验完成，有警告
verify-verdict-fail = { $name } — 发现问题
verify-container = 容器
verify-video-continuity = 视频连续性
verify-audio-continuity = 音频连续性
verify-av-interleave = 音视频交织
verify-duration = 时长
recordings-alpha-label = Alpha
recordings-prores-title = 导出保留 Alpha 的 ProRes 4444 .mov 母版（用于剪辑）
recordings-qtrle-title = 导出保留 Alpha 的 QuickTime Animation .mov（兼容性最强）
trim-title = 修剪 — { $name }
trim-loading = 正在读取文件…
trim-preview-alt = 预览帧
trim-position = 播放位置
trim-step-second-back = 后退一秒
trim-step-frame-back = 后退一帧
trim-step-frame-forward = 前进一帧
trim-step-second-forward = 前进一秒
trim-snap = 关键帧
trim-snap-title = 吸附到最近的关键帧 — 在此处剪切可无需重新编码导出
trim-set-in = 设入点
trim-set-out = 设出点
trim-range-invalid = 出点必须在入点之后。
trim-copy-badge = ✓ 无需重新编码导出 — 入点正好位于关键帧上。
trim-reencode-badge = 将重新编码：入点位于关键帧之间（用“关键帧”吸附可实现无损剪切）。
trim-export = 导出片段
trim-export-916 = 9:16
trim-export-916-title = 导出竖版重构图（按竖版画布尺寸居中裁剪）— 始终重新编码
...
bench-open = 运行编码器基准测试…
bench-title = 编码器基准测试
bench-intro = 在本机运行简短的实测编码阶梯（每个检测到的编码器 × 预设 × 分辨率）— 约一分钟，完全离线，不会有任何数据离开电脑。失败会列出，绝不隐藏。请先停止直播或录制。
bench-start = 开始测试
bench-rerun = 再次运行
bench-running = 测量中… { $done } / { $total }
bench-cancel = 取消
bench-col-encoder = 编码器
bench-col-preset = 预设
bench-col-rung = 档位
bench-col-achieved = fps
bench-col-headroom = 余量
bench-failed = 失败
bench-rec-title = 推荐（实测）
bench-rec-body = { $encoder }（{ $preset }），{ $width }×{ $height } @ { $fps } fps — 实测 { $headroom }× 实时。建议直播码率：{ $bitrate } kbps。
bench-rec-none = 本机没有任何设置能维持实时 — 请降低画布分辨率或 fps 后重新测量。
bench-apply = 应用到录制设置
bench-applied = 已应用 ✓
...
output-iso-heading = ISO 录制
output-iso-explainer = 将选中的源以干净状态各自录制到独立文件，与节目并行 — 在合成之前，按画布尺寸和帧率记录，因此每个文件都能对齐地放进剪辑时间线。中端 GPU 上两条通道运行自如；每增加一条通道就多一次渲染和编码。
output-iso-none = 合集中还没有源。
output-iso-source-on = 正在将“{ $name }”录制为独立 ISO 文件 — 点击停止
output-iso-source-off = 将“{ $name }”录制为独立 ISO 文件
output-iso-post-filter = 携带源的滤镜录制（滤镜后）；不勾选则录制原始源
output-iso-format = ISO 格式
output-iso-encoder = ISO 视频编码器
output-alpha-frec = 带透明度（Alpha）录制 — 节目位于透明背景之上
output-auto-export = 同时在母带旁保存一个可分享的 MP4
output-auto-export-title = 当 .frec 录制完成时，后台导出会在其旁写入一个双击即可播放的 MP4 副本（可在后台队列中查看）。无损母带仍是母带。
output-alpha-title = 录制器获得独立的透明渲染；预览和推流保持不变。可从录像列表导出为 ProRes 4444 或 QTRLE — MP4/MKV 会压平 Alpha。
output-split-events = 在以下时刻也开始新文件…（每个分段恰好从事件处开始；最短时长 1 秒）
output-split-on-scene = 切换场景
output-split-on-marker = 标记
output-split-on-rundown = 流程步骤
output-auto-markers = 在工作室事件时自动打章节标记（切换场景、保存回放、重连、掉帧、警报、规则）
output-auto-markers-title = 带类型的标记会写入录像章节（mkv）或 .chapters.txt 附属文件，与手动标记热键并存
output-pipeline-heading = 录制后流水线
output-pipeline-explainer = 录像完成后，这些步骤会按顺序在后台作用于主文件。封闭的动作集 — 有意不提供"运行命令"步骤。链条在首个失败处停止。
output-pipeline-enabled = 每次录制后运行流水线
output-pipeline-add = 添加步骤…
output-pipeline-up = 上移
output-pipeline-down = 下移
output-pipeline-remove = 移除步骤
output-pipeline-template = 重命名模板（CAP-M25 令牌）
output-pipeline-folder = 文件夹
pipeline-queue = 录制后流水线
pipeline-verify = 校验
pipeline-remux = 重封装为 MP4
pipeline-normalize = 响度归一化
pipeline-rename = 重命名
pipeline-move = 移动到文件夹
pipeline-copy = 复制到文件夹
pipeline-reveal = 在文件管理器中显示
pipeline-luaEvent = 通知 Lua 脚本
output-presets = 预设：
...
stream-session-report = 会话结束时在录制文件旁写入会话报告（HTML + Markdown）
stream-simulator-title = 网络模拟器（彩排）
stream-simulator-note = 只作用于彩排的本地接收端 — 演练重连和弱上行。真实开播绝不会被降级。
stream-simulator-profile = 配置
stream-simulator-off = 关闭
stream-simulator-hotel-wifi = 酒店 Wi-Fi
stream-simulator-mobile-hotspot = 手机热点
stream-simulator-custom = 自定义
stream-simulator-bandwidth = 带宽（kbps，0 = 不限）
stream-simulator-latency = 延迟（ms）
stream-simulator-jitter = 抖动（± ms）
stream-simulator-outage-every = 每隔多久断开（秒，0 = 从不）
stream-simulator-outage-len = 断开时长（秒）
...
workspace-pack-title = 场景集合包
workspace-pack-export = 导出集合包…
workspace-pack-exporting = 正在导出…
workspace-pack-import = 导入集合包…
workspace-pack-importing = 正在导入…
workspace-pack-hint = 一个 .fcappack 文件将活动集合及其本地素材打包为单个便携文件 — 分享整套布局，无需服务器。导入会将其作为新集合载入并切换过去。
workspace-pack-exported = 已导出"{ $name }" — { $bundled } 个素材已打包，{ $external } 个为外部。
workspace-pack-imported = 已导入"{ $name }" — { $scenes } 个场景、{ $relinked } 个素材已重新链接、{ $external } 个为外部。
workspace-compare = 比较并合并…
compare-title = 比较并合并
compare-intro = 将当前集合与另一个集合比较，只引入你选择的更改。任何内容都不会离开本机。
compare-target-label = 比较对象
compare-target-none = 选择一个集合…
compare-target-pack = 从集合包文件…
compare-against = 对比 { $name }
compare-comparing = 正在比较…
compare-no-diff = 没有差异 — 两个集合一致。
compare-select-all = 全选
compare-clear = 清除
compare-merge-hint = 选择场景时会一并引入它使用的来源。
compare-sources-title = 来源
compare-scenes-title = 场景
compare-kind-added = 已添加
compare-kind-removed = 已移除
compare-kind-modified = 已更改
compare-renamed = 原为"{ $from }"
compare-reordered = 已重新排列
compare-apply = 应用 { $count } 项
compare-apply-none = 选择要应用的更改
compare-applying = 正在应用…
compare-aspect-source = 来源
compare-aspect-transform = 位置
compare-aspect-visibility = 可见性
compare-aspect-outputs = 输出
compare-aspect-filters = 滤镜
compare-aspect-lock = 锁定
compare-aspect-blend = 混合
compare-aspect-scaling = 缩放
compare-aspect-backdrop = 背景
compare-aspect-reveal = 揭示
compare-aspect-name = 名称
compare-aspect-settings = 设置
compare-aspect-audio = 音频
workspace-snapshots = 快照…
snapshots-title = 快照
snapshots-intro = 此集合的命名检查点，可随时回到 — 有意保留，超越自动保存与撤销。存储在本地。
snapshots-name-placeholder = 为此检查点命名…
snapshots-create = 保存快照
snapshots-empty = 还没有快照 — 保存一个，留下可随时回到的命名检查点。
snapshots-usage = 已保留 { $count } / { $cap } · { $size }
snapshots-scenes = { $scenes } 个场景 · { $sources } 个源
snapshots-restore = 恢复
snapshots-compare = 比较
snapshots-delete = 删除快照
snapshots-delete-aria = 删除快照"{ $name }"
snapshots-restore-confirm = 恢复此快照？将先把当前布局保存为一个快照。

# --- Phase 8 (backup / theme editor / dock presets / settings search / quick actions) ---
workspace-backup = 备份与恢复…
backup-title = 备份与恢复
backup-intro = 将整个工作室 — 设置、配置文件、场景集合 — 保存到一个文件，并在此处或新机器上恢复。
backup-export = 导出备份…
backup-exporting = 正在导出…
backup-exported = 已备份 { $collections } 个集合和 { $profiles } 个配置文件（{ $size }）。
backup-restore = 从备份恢复…
backup-secrets-note = 备份中绝不包含推流密钥和密码 — 恢复后请重新输入。
backup-restore-from = 备份创建于 { $created }（v{ $version }）
backup-restore-settings = 设置
backup-restore-collections = 场景集合（{ $count }）
backup-restore-profiles = 配置文件（{ $count }）
backup-restore-apply = 恢复所选
backup-restoring = 正在恢复…
backup-restored = 已恢复。
backup-restart-note = 重启 Freally Capture 以加载已恢复的集合和配置文件。
backup-cancel = 取消
theme-editor-title = 自定义调色板
theme-color-bg = 背景
theme-color-panel = 面板
theme-color-text = 文字
theme-color-muted = 弱化色
theme-color-accent = 强调色
theme-color-accent2 = 强调色 2
theme-contrast-text-bg = 背景上的文字
theme-contrast-text-panel = 面板上的文字
theme-contrast-muted-bg = 背景上的弱化色
theme-contrast-accent-bg = 背景上的强调色
theme-contrast-low = （目标 ≥ { $min }:1）
theme-reset = 重置为深色
theme-export = 导出 .fctheme…
theme-import = 导入 .fctheme…
workspace-dock-presets = 停靠布局
workspace-dock-preset-placeholder = 将当前布局另存为…
workspace-dock-preset-save = 保存
workspace-dock-preset-apply = 切换到"{ $name }"
workspace-dock-preset-delete = 删除布局"{ $name }"
workspace-dock-presets-hint = 布局会记住统计停靠窗和混音器方向。点击某个布局即可切换。
settings-search-placeholder = 搜索设置…
settings-search-none = 没有匹配的设置。
settings-changed = 打开后已更改
menu-tools-quick-actions = 快捷操作…
menu-tools-featured-chat = 精选聊天消息…
quick-actions-title = 快捷操作
quick-actions-empty = 还没有按钮 — 点击"编辑"添加。
quick-actions-edit = 编辑
quick-actions-done = 完成
quick-actions-add-page = 页面
quick-actions-label = 按钮标签
quick-actions-color = 按钮颜色
quick-actions-add = 添加按钮
quick-actions-remove = 移除"{ $name }"
quick-actions-delete-page = 删除"{ $name }"
quick-actions-hint = 按钮触发与热键和 LAN 面板相同的工作室命令 — 场景、宏和音效板按键也一样。命令按钮也会显示在 LAN 面板上。
quick-actions-cmd-transition = 转场
quick-actions-cmd-startStream = 开始直播
quick-actions-cmd-stopStream = 结束直播
quick-actions-cmd-startRecording = 开始录制
quick-actions-cmd-stopRecording = 停止录制
quick-actions-cmd-pauseRecording = 暂停录制
quick-actions-cmd-addMarker = 添加标记
quick-actions-cmd-armReplay = 启用重放
quick-actions-cmd-saveReplay = 保存重放
quick-actions-cmd-setStudioMode = 切换工作室模式
quick-actions-cmd-scene = 切换到场景…
quick-actions-cmd-macro = 运行宏…
quick-actions-cmd-soundboard = 音效板按键…
quick-actions-value-scene = 场景名称
quick-actions-value-macro = 宏名称
quick-actions-value-soundboard = 按键 ID
teleprompter-stop = 停止
about-portable-on = 便携模式：开 — 所有应用数据都存储在应用旁边。
about-portable-off = 便携模式：关。在应用旁边添加一个"{ $marker }"文件，即可完全独立运行（U 盘模式）。
about-portable-data = 数据文件夹：{ $path }
...
history-toggleOutputVisibility = 切换输出可见性
...
palette-featured-chat = 精选聊天消息…
featured-chat-title = 精选聊天消息
featured-chat-intro = 将任意最近的聊天内容固定为节目底部横幅——直播和录制的观众都能看到。固定在重启后清除；颜色会保留。
featured-chat-empty = 还没有聊天——添加聊天叠加源并指向你的频道；消息到达后会显示在这里。
featured-chat-pinned = 节目中：
featured-chat-pin = 精选
featured-chat-clear = 清除
featured-chat-bg = 横幅颜色
featured-chat-text = 文字颜色
featured-chat-note = 横幅位于底部居中、所有场景之上——清除即可下线。
health-title = 源健康状态
health-col-source = 源
health-col-state = 状态
...
properties-stats-show-timecode = 显示时间码（LTC）
...
sources-visualizer-classic = 经典配色
...
menu-bar-label = 应用程序菜单
menu-file = 文件
menu-edit = 编辑
menu-view = 视图
menu-docks = 停靠窗
menu-profile = 配置文件
menu-collection = 场景集合
menu-tools = 工具
menu-help = 帮助
menu-rename = 重命名
menu-remove = 移除
menu-import = 导入
menu-export = 导出
menu-file-show-recordings = 显示录制文件
menu-file-remux = 重封装为 MP4…
menu-file-settings = 设置…
menu-file-show-settings-folder = 显示设置文件夹
menu-file-exit = 退出
menu-edit-undo = 撤销
menu-edit-redo = 重做
menu-edit-history = 编辑历史…
menu-edit-copy-transform = 复制变换
menu-edit-paste-transform = 粘贴变换
menu-edit-copy-filters = 复制滤镜
menu-edit-paste-filters = 粘贴滤镜
menu-edit-transform = 变换…
menu-edit-lock-preview = 锁定预览
menu-view-fullscreen = 全屏界面
menu-stats-dock = 统计面板
menu-view-multiview = 多画面预监…
menu-view-projectors = 投影…
menu-view-source-health = 源健康状态…
menu-view-still = 抓取静帧
menu-docks-browser = 浏览器停靠窗…
menu-docks-lock = 锁定停靠窗
menu-docks-reset = 重置停靠窗布局
menu-profile-manage = 管理配置文件…
menu-collection-manage = 管理场景集合…
menu-collection-import-obs = 从 OBS 导入…
menu-collection-missing = 检查缺失文件…
menu-collection-clear = 清除历史…
menu-collection-clear-confirm = 永久删除这些场景集合及其文件？{ $names } — 保留当前使用的集合"{ $active }"。此操作无法撤销。
menu-tools-wizard = 运行设置向导
menu-tools-wizard-title = 设置向导在首次启动时运行；暂时无法重新运行。
menu-tools-automation = 自动化规则与宏…
menu-tools-rundown = 显示节目流程单…
menu-tools-hotkeys = 快捷键总览…
menu-tools-av-sync = A/V 同步校准…
menu-tools-scripts = Lua 脚本…
menu-tools-components = 组件…
menu-tools-midi = MIDI 控制…
menu-tools-ptz = PTZ 摄像机…
menu-tools-remote = 远程控制 API…
menu-tools-panel = 局域网面板与 Tally…
menu-help-portal = 帮助门户
menu-help-website = 访问网站
menu-help-discord = 加入 Discord 服务器
menu-help-bug = 报告错误…
...
settings-cat-streaming = 推流
settings-cat-output = 输出
settings-cat-replay = 重放
settings-cat-hotkeys = 热键
settings-cat-network = 网络
settings-cat-accessibility = 辅助功能
...
settings-hotkeys-filter = 筛选热键
settings-hotkeys-filter-placeholder = 输入以筛选操作或按键…
settings-hotkeys-no-match = 没有与“{ $query }”匹配的热键。
settings-hotkey-none = 无
settings-hotkey-group-ctrl = Ctrl + 按键
settings-hotkey-group-ctrl-shift = Ctrl + Shift + 按键
settings-hotkey-group-ctrl-alt = Ctrl + Alt + 按键
settings-hotkey-group-function = 功能键
settings-hotkey-group-numpad = 小键盘
settings-panic-section = 隐私画面
settings-meter-section = 混音器电平表
settings-meter-note = 音频混音器电平表从安静到削波所经过的颜色。色盲友好预设使用蓝色 → 橙色渐变，在红绿色觉障碍下也能分辨。
settings-meter-preset = 电平表颜色
settings-meter-preset-default = 绿 / 黄 / 红
settings-meter-preset-colorblind = 色盲友好（蓝 / 橙）
settings-meter-preset-custom = 自定义
settings-meter-low = 正常
settings-meter-mid = 响亮
settings-meter-high = 削波
settings-meter-preview = 预览

# --- CAP-N: What's New, blur/pixelate/freeze filters, 3D transform, clone, Downstream Keyers ---
whats-new-title = 新增功能
whats-new-loading = 正在加载发行说明…
whats-new-version = 版本 { $version } 的新增功能
whats-new-empty = 此版本没有发行说明。
filters-name-directional-blur = 方向模糊
filters-name-radial-blur = 径向模糊
filters-name-zoom-blur = 缩放模糊
filters-name-pixelate = 像素化
filters-angle = 角度 (°)
filters-center-x = 中心 X
filters-center-y = 中心 Y
filters-block-size = 块大小 (px)
filters-name-freeze = 冻结
filters-freeze-hint = 启用后，此源将保持其最后一帧 — 节目、预览、录制和推流将一起冻结。切换此滤镜以冻结或解冻。
transform-3d = 3D 倾斜
...
# --- Telestrator (CAP-N57) ---
telestrator-group = 战术板
telestrator-draw = 绘制
telestrator-tool-pen = 画笔
telestrator-tool-highlight = 荧光笔
telestrator-tool-arrow = 箭头
telestrator-tool-ellipse = 椭圆
telestrator-color = 颜色
telestrator-width = 粗细
telestrator-whiteboard = 白板模式
telestrator-persist = 保留
telestrator-fade = 淡出
telestrator-undo = 撤销上一笔
telestrator-clear = 清除标记
hotkeys-telestrator-clear = 战术板：清除标记

# --- Teleprompter (CAP-N58) ---
teleprompter-title = 提词器
teleprompter-loading = 加载中…
teleprompter-empty = 还没有稿件 — 在提词器面板中添加一份。
teleprompter-script = 稿件
teleprompter-script-placeholder = 输入或粘贴你的稿件…
teleprompter-top = 顶部
teleprompter-slower = 更慢
teleprompter-play = 播放
teleprompter-pause = 暂停
teleprompter-faster = 更快
teleprompter-step-back = 后退
teleprompter-step-forward = 前进
teleprompter-est-time = 朗读时间
teleprompter-caesura-hint = 输入 -- 表示停顿
teleprompter-caesura-pause = 停顿 — 输入 -- 表示停顿
teleprompter-countdown = 开始倒计时
teleprompter-countdown-secs = 倒计时（秒）
teleprompter-speed = 速度（字符/秒）
teleprompter-speed-bpm = 速度（BPM）
teleprompter-bpm-mode = BPM 模式（歌唱）
teleprompter-font = 字号
teleprompter-mirror = 镜像（玻璃）
teleprompter-open-projector = 打开投影
teleprompter-preview = 预览
teleprompter-remote-hint = 关闭此面板后，可用快捷键、MIDI 或局域网面板控制滚动。
teleprompter-read-aloud = 使用各操作系统的语音合成朗读
menu-tools-teleprompter = 提词器
hotkeys-teleprompter-toggle = 提词器：播放 / 暂停

# --- Remote guests: green room / cues / QoS / auto-grid (CAP-N54–N56, N59) ---
remote-hosting-count = 正在主持 { $count } 位嘉宾
remote-green-room-default = 候场室
remote-green-room-default-title = 新嘉宾在候场室等待，直到你安排就位
remote-auto-grid = 自动网格
remote-auto-grid-title = 嘉宾进出时自动保持网格排列
remote-arrange-grid = 排列网格
remote-arrange-grid-title = 立即把在场嘉宾重排为网格
remote-green-room-monitor = 候场预览
remote-tech-cam-ok = 摄像头 ✓
remote-tech-cam-no = 无摄像头
remote-tech-mic-ok = 麦克风 ✓
remote-tech-mic-no = 无麦克风
remote-seat-on-air = 上镜
remote-cues-label = 提示
remote-cue = 提示
remote-cue-thirty = 30 秒
remote-cue-wrap = 准备收尾
remote-cue-next = 下一个就是你
remote-cue-speak = 大点声
remote-qos-good = 连接良好
remote-qos-fair = 连接一般
remote-qos-poor = 连接较差
remote-green-room-guest = 你在候场室 — 主持人稍后会让你上镜。

# --- Hotkey audit — Phase 7 actions ---
hotkey-audit-action-telestrator-clear = 战术板：清除标记
hotkey-audit-action-teleprompter-toggle = 提词器：播放 / 暂停
hotkey-audit-feature-telestrator = 战术板
hotkey-audit-feature-teleprompter = 提词器

# V1-C — Starting Soon countdown slate
sources-add-starting-soon = 马上开始…
sources-starting-soon-title = 马上开始
sources-starting-soon-default = 马上开始
sources-starting-soon-message = 消息
sources-starting-soon-minutes = 分钟
sources-starting-soon-hours = 小时
sources-starting-soon-start-at = 开始时间
sources-starting-soon-music = 背景音乐…
sources-starting-soon-music-name = 背景音乐
sources-starting-soon-background = 背景
sources-starting-soon-note = 全屏倒计时，归零时红色闪烁并停住——准备好后手动清除并切到你的直播场景。
sources-starting-soon-add = 添加
sources-slate-solid = 纯色
sources-slate-gradient = 渐变
sources-slate-transparent = 透明
sources-slate-image = 图片
sources-slate-file = 图片 / 视频 / GIF
sources-slate-browse = 浏览…
sources-starting-soon-choose-image = 选择图片…
sources-slate-media-note = 静态图片在此嵌入。若要循环视频或 GIF 背景，请选择“透明”并设置场景的背景幕（场景栏中的 🖼）——倒计时显示在最上层。
properties-timer-slate = “马上开始”幕布
properties-timer-slate-off = 关闭（内嵌文字）
properties-timer-message = 消息
properties-timer-slate-color = 幕布颜色
properties-timer-slate-from = 起始
properties-timer-slate-to = 结束
properties-timer-slate-note = 透明可让幕布下方的图片、视频或场景背景透出。
...
sources-badge-browser = 浏览器
sources-add-browser = 浏览器（网页）
sources-browser-title = 添加浏览器来源
sources-browser-add = 添加浏览器来源
sources-browser-url = 页面 URL（http:// 或 https://）
sources-browser-url-invalid = 请输入 http:// 或 https:// 开头的 URL — 本地文件请使用媒体或图像来源播放。
sources-browser-width = 宽度 (px)
sources-browser-height = 高度 (px)
