...
sources-add-background-music = Background Music…
sources-background-music-note = Loops, heard in preview and captured to your recording/stream. To use music instead of a video's sound, mute the video's Backdrop strip in the mixer.
sources-background-music-count = { $n } tracks
sources-background-music-shuffle = Shuffle
...
sources-stream-hide = Hide on stream
sources-stream-show = Show on stream
sources-stream-hide-item = Hide { $name } on stream
sources-stream-show-item = Show { $name } on stream
sources-record-hide = Hide in recording
sources-record-show = Show in recording
sources-record-hide-item = Hide { $name } in recording
sources-record-show-item = Show { $name } in recording
...
sources-clone-title = Clone (same feed, own filters)
sources-clone-item = Clone { $name }
...
sources-game-accept = Accept the risk & choose a game
sources-game-pick = Pick the game window to capture. It stays hooked only while you play it.
...
sources-visualizer-classic = Classic colors
...
controls-iso-lanes = ISO lanes recording alongside the program: { $count }
...
mixer-routing-title = Audio output routing

# --- RoutingMatrixDialog.tsx (CAP-N30) ---
routing-title = Audio routing
routing-intro = Assign strips to track buses, then send any bus to a physical output — a hardware-recorder feed, speakers in another room, or a headphone cue on a spare track. The monitor keeps its own device; these routes are added on top, so with none set the mix is unchanged.
routing-sends-title = Track sends
routing-no-strips = No audio sources in this scene.
routing-source = Source
routing-track = Track { $n }
routing-send-aria = Send { $source } to track { $n }
routing-outputs-title = Physical outputs
routing-master = Master
routing-off = Off
routing-default-output = Default output
routing-device-aria = Output device for { $bus }
routing-trim-aria = Output trim for { $bus }
routing-trim-db = { $db } dB
routing-muted = Muted
routing-device-error = Device unavailable

# --- DuckingMatrixDialog.tsx (CAP-N31) ---
mixer-ducking = Ducking
mixer-ducking-title = Ducking matrix
ducking-title = Ducking matrix
ducking-intro = Any source can duck any others. A cell dips the target (column) whenever the trigger (row) speaks — pick a cell to set its depth, threshold, and timing. Each pair is its own duck, so one strip can be ducked by several triggers at once.
ducking-need-two = Add at least two audio sources to duck between them.
ducking-trigger-target = Trigger ↓ / Target →
ducking-cell-aria = { $trigger } ducks { $target }
ducking-pair = { $trigger } → { $target }
ducking-remove = Remove
ducking-amount = Amount
ducking-threshold = Threshold
...
loudness-title = Loudness normalization
loudness-intro = Ride the program toward a loudness target with a peak ceiling, so your stream and recordings land at a consistent level. Slow and gentle — it steers, never pumps.
loudness-enable = Ride the program to the target
loudness-target = Target
loudness-target-option = { $target } LUFS
loudness-ceiling = Peak ceiling (dBFS)
loudness-note = −14 LUFS suits YouTube-style playback; −16 is a common streaming target; −23 is EBU R128 broadcast. The same target is used by the post-record Normalize action.
ltc-badge = LTC
ltc-title = SMPTE Timecode (LTC)
ltc-intro = Generate SMPTE linear timecode on a track, and read incoming LTC from any audio input — classic audio timecode for syncing external recorders and cameras in post. Fully offline.
ltc-generate = Generate LTC onto a track
ltc-track = Timecode track
ltc-track-option = Track { $track }
ltc-fps = Frame rate
ltc-read = Read LTC from
ltc-read-off = Off
ltc-decoded = Incoming timecode
ltc-no-lock = no signal
ltc-note = The generator jam-syncs to the time of day, non-drop. Record its track (assign it in Output settings) or route it to an output to feed external gear. The reader drives the stats-overlay timecode line and stamps chapter markers.
...
soundboard-stop-all = Stop all
soundboard-edit = Edit
soundboard-empty = No pads yet — add one and assign a local audio clip.
soundboard-new-pad = New pad
soundboard-no-clip = No clip
soundboard-audio-files = Audio files
soundboard-name = Name
soundboard-choose-clip = Choose clip…
soundboard-gain = Gain
soundboard-choke = Choke
soundboard-choke-none = None
soundboard-loop = Loop
soundboard-auto-duck = Auto-duck
soundboard-tracks = Tracks
soundboard-hotkey = Hotkey
soundboard-hotkey-placeholder = e.g. Ctrl+Shift+1
...
plugins-scanning = Scanning…
plugins-none = No CLAP or VST3 plugins found in the standard folders.
...
stats-disk-left = Rec left
stats-disk-rate = ≈ { $rate } MB/s recording
stats-vertical-fps = 9:16 FPS
stats-targets-label = Stream targets
stats-rehearsal-note = Rehearsal — targets publish to a local sink only
stats-timeline-open = Timeline
timeline-title = Session timeline
timeline-empty = Nothing recorded yet — the timeline records while you stream or record.
timeline-live = LIVE — still recording
timeline-fit = Fit
timeline-legend-fps = fps
timeline-legend-behind = encoder queue (frames behind)
...
scenes-recent-title = Recent
scenes-recent-clear = Clear
scenes-recent-clear-confirm = Clear the recent scenes list?
...
channelstrip-automix-note = Gain-sharing: the mixer keeps the combined level of all auto-mixed strips steady and hands it to whoever is speaking — ideal for multi-mic panels and podcasts. Off until you add a strip.
channelstrip-mix-minus-label = Mix-minus (N−1)
channelstrip-mix-minus-note = Produce an echo-free return for this source — everyone in the program except this source itself. Use it for a remote guest so they don't hear their own delayed voice.
...
livebutton-rehearse = Rehearse
livebutton-rehearse-record-confirm = Also record this rehearsal to a video file?
livebutton-rehearse-title = Run the whole show against a local sink — nothing is sent
livebutton-end-rehearsal = End rehearsal
livebutton-title-rehearsing = A rehearsal is running — nothing leaves this machine
livebutton-badge-rehearsal = REHEARSAL
livebutton-aria-rehearsal = Rehearsing
livebutton-rehearsal-banner = Rehearsal — nothing leaves this machine
...
properties-timer-clock = Count down to a clock time
...
properties-stats-show-timecode = Show timecode (LTC)
...
audiofilters-name-parametric-eq = Parametric EQ
...
audiofilters-voice-preset-pick = Voice preset…
audiofilters-voice-broadcast = Broadcast voice
audiofilters-voice-podcast = Podcast voice
audiofilters-voice-clean = Clean voice
audiofilters-voice-none = Clear chain
# --- De-esser + rumble guard params (CAP-N36) ---
audiofilters-deesser-freq = Sibilance freq (Hz)
audiofilters-deesser-amount = Max reduction (dB)
audiofilters-rumble-freq = Low-cut (Hz)
audiofilters-title = Audio filters — { $name }

# --- ParametricEqEditor.tsx (CAP-N35) ---
eq-graph-aria = Parametric EQ response curve with a live spectrum
eq-band-type = Type
eq-freq = Hz
eq-gain = dB
eq-q = Q
eq-add-band = + Band
eq-remove-band = Remove band
eq-type-bell = Bell
...
filters-name-fade-loop = Fade Loop
filters-name-crop = Crop
filters-name-directional-blur = Directional Blur
filters-name-radial-blur = Radial Blur
...
filters-tilt = Tilt
filters-far-fade = Far-edge fade
filters-fade-in-s = Fade in (s)
filters-visible-s = Visible (s)
filters-fade-out-s = Fade out (s)
filters-hidden-s = Hidden (s)
...
recordings-trim = Trim
recordings-trim-title = Cut a clip out of this recording — keyframe-aligned cuts export without re-encoding
recordings-verify = Verify
recordings-verify-title = Check the file's integrity — container structure, continuity, A/V interleave, duration
recordings-verifying = Verifying…
verify-dismiss = Dismiss
verify-verdict-pass = { $name } — integrity OK
verify-verdict-warn = { $name } — verified with warnings
verify-verdict-fail = { $name } — problems found
verify-container = Container
verify-video-continuity = Video continuity
verify-audio-continuity = Audio continuity
verify-av-interleave = A/V interleave
verify-duration = Duration
recordings-alpha-label = alpha
recordings-prores-title = Export an alpha-preserving ProRes 4444 .mov master (for editing)
recordings-qtrle-title = Export an alpha-preserving QuickTime Animation .mov (maximal compatibility)
trim-title = Trim — { $name }
trim-loading = Reading the file…
trim-preview-alt = Preview frame
trim-position = Playback position
trim-step-second-back = Back one second
trim-step-frame-back = Back one frame
trim-step-frame-forward = Forward one frame
trim-step-second-forward = Forward one second
trim-snap = Keyframe
trim-snap-title = Snap to the nearest keyframe — a cut there exports without re-encoding
trim-set-in = Set in
trim-set-out = Set out
trim-range-invalid = The out-point must come after the in-point.
trim-copy-badge = ✓ Exports without re-encoding — the in-point sits on a keyframe.
trim-reencode-badge = Will re-encode: the in-point is between keyframes (use Keyframe to snap for a lossless cut).
trim-export = Export clip
trim-export-916 = 9:16
trim-export-916-title = Export reframed vertical (center crop through the vertical-canvas size) — always re-encodes
...
recordings-normalize = Normalize
recordings-normalizing = Normalizing…
recordings-normalize-title = Normalize loudness to the target (writes a copy)
recordings-normalized-to = Normalized to { $path }

# --- Audio-only recording (CAP-N38) ---
audiorec-title = Audio-only
audiorec-format = Audio recording format
audiorec-format-wav = WAV
audiorec-format-flac = FLAC
audiorec-format-opus = Opus
audiorec-start = Record audio
audiorec-stop = Stop
audiorec-pause = Pause
audiorec-resume = Resume
audiorec-recording = REC { $sec }s
audiorec-saved = Saved { $count } track file(s)
...
bench-open = Run encoder benchmark…
bench-title = Encoder benchmark
bench-intro = Runs short measured encode ladders (every detected encoder × preset × resolution) on this machine — about a minute, fully offline, nothing leaves your computer. Failures are listed, not hidden. Stop any stream or recording first.
bench-start = Start benchmark
bench-rerun = Run again
bench-running = Measuring… { $done } / { $total }
bench-cancel = Cancel
bench-col-encoder = Encoder
bench-col-preset = Preset
bench-col-rung = Rung
bench-col-achieved = fps
bench-col-headroom = Headroom
bench-failed = failed
bench-rec-title = Recommendation (measured)
bench-rec-body = { $encoder } at { $preset }, { $width }×{ $height } @ { $fps } fps — measured { $headroom }× realtime. Suggested stream bitrate: { $bitrate } kbps.
bench-rec-none = Nothing sustains realtime on this machine — lower the canvas resolution or fps and run again.
bench-apply = Apply to recording settings
bench-applied = Applied ✓
...
output-iso-heading = ISO recording
output-iso-explainer = Record selected sources clean, each to its own file alongside the program — pre-composite, at canvas size and frame rate, so every file drops onto an editing timeline aligned. Two lanes are comfortable on a mid-range GPU; each extra lane costs another render and encode.
output-iso-none = No sources in the collection yet.
output-iso-source-on = Recording “{ $name }” as its own ISO file — click to stop
output-iso-source-off = Record “{ $name }” as its own ISO file
output-iso-post-filter = Record with the source's filters (post-filter); unchecked records the raw source
...
output-split-on-marker = marker
output-split-on-rundown = rundown step
output-auto-markers = Drop chapter markers automatically on studio events (scene switch, replay save, reconnect, dropped frames, alarms, rules)
output-auto-markers-title = Typed markers land in the recording's chapters (mkv) or the .chapters.txt sidecar, alongside the manual marker hotkey
output-pipeline-heading = Post-record pipeline
output-pipeline-explainer = After a recording finalizes, run these steps on the main file, in order, in the background. A closed action set — there is no "run a command" step, by design. The chain stops at the first failure.
output-pipeline-enabled = Run the pipeline after every recording
output-pipeline-add = Add a step…
output-pipeline-up = Move up
output-pipeline-down = Move down
output-pipeline-remove = Remove step
output-pipeline-template = Rename template (CAP-M25 tokens)
output-pipeline-folder = Folder
pipeline-queue = Post-record pipeline
pipeline-verify = Verify
pipeline-remux = Remux to MP4
pipeline-normalize = Normalize loudness
pipeline-rename = Rename
pipeline-move = Move to folder
pipeline-copy = Copy to folder
pipeline-reveal = Reveal in file manager
pipeline-luaEvent = Notify Lua scripts
...
stream-session-report = Write a session report (HTML + Markdown) next to the recording when the session ends
stream-simulator-title = Network simulator (rehearsals)
stream-simulator-note = Shapes only the rehearsal's local sinks — drills reconnects and thin uplinks. A real Go Live is never degraded.
stream-simulator-profile = Profile
stream-simulator-off = Off
stream-simulator-hotel-wifi = Hotel Wi-Fi
stream-simulator-mobile-hotspot = Mobile hotspot
stream-simulator-custom = Custom
stream-simulator-bandwidth = Bandwidth (kbps, 0 = uncapped)
stream-simulator-latency = Latency (ms)
stream-simulator-jitter = Jitter (± ms)
stream-simulator-outage-every = Outage every (s, 0 = never)
stream-simulator-outage-len = Outage length (s)
...
workspace-pack-title = Scene-collection pack
workspace-pack-export = Export pack…
workspace-pack-exporting = Exporting…
workspace-pack-import = Import pack…
workspace-pack-importing = Importing…
workspace-pack-hint = A .fcappack bundles the active collection plus its local assets into one portable file — share a whole layout, no server. Import lands it as a new collection and switches to it.
workspace-pack-exported = Exported "{ $name }" — { $bundled } asset(s) bundled, { $external } external.
workspace-pack-imported = Imported "{ $name }" — { $scenes } scene(s), { $relinked } asset(s) relinked, { $external } external.
workspace-compare = Compare & merge…
compare-title = Compare & merge
compare-intro = Compare the active collection against another and pull in only the changes you pick. Nothing leaves this machine.
compare-target-label = Compare against
compare-target-none = Choose a collection…
compare-target-pack = From a pack file…
compare-against = vs { $name }
compare-comparing = Comparing…
compare-no-diff = No differences — the collections match.
compare-select-all = Select all
compare-clear = Clear
compare-merge-hint = Picking a scene also brings the sources it uses.
compare-sources-title = Sources
compare-scenes-title = Scenes
compare-kind-added = Added
compare-kind-removed = Removed
compare-kind-modified = Changed
compare-renamed = was "{ $from }"
compare-reordered = restacked
compare-apply = Apply { $count }
compare-apply-none = Select changes to apply
compare-applying = Applying…
compare-aspect-source = source
compare-aspect-transform = position
compare-aspect-visibility = visibility
compare-aspect-outputs = outputs
compare-aspect-filters = filters
compare-aspect-lock = lock
compare-aspect-blend = blend
compare-aspect-scaling = scaling
compare-aspect-backdrop = backdrop
compare-aspect-reveal = reveal
compare-aspect-name = name
compare-aspect-settings = settings
compare-aspect-audio = audio
workspace-snapshots = Snapshots…
snapshots-title = Snapshots
snapshots-intro = Named checkpoints of this collection you can return to — kept on purpose, beyond autosave and undo. Stored locally.
snapshots-name-placeholder = Name this checkpoint…
snapshots-create = Save snapshot
snapshots-empty = No snapshots yet — save one to keep a named checkpoint you can return to.
snapshots-usage = { $count } of { $cap } kept · { $size }
snapshots-scenes = { $scenes } scenes · { $sources } sources
snapshots-restore = Restore
snapshots-compare = Compare
snapshots-delete = Delete snapshot
snapshots-delete-aria = Delete snapshot "{ $name }"
snapshots-restore-confirm = Restore this snapshot? Your current layout is saved as a snapshot first.
about-portable-on = Portable mode: on — all app data is stored next to the app.
about-portable-off = Portable mode: off. Add a "{ $marker }" file next to the app to run fully self-contained (USB-stick mode).
about-portable-data = Data folder: { $path }
workspace-backup = Backup & restore…
backup-title = Backup & restore
backup-intro = Save your whole studio — settings, profiles, scene collections — to one file, and restore it here or on a new machine.
backup-export = Export backup…
backup-exporting = Exporting…
backup-exported = Backed up { $collections } collection(s) and { $profiles } profile(s) ({ $size }).
backup-restore = Restore from backup…
backup-secrets-note = Stream keys and passwords are never included in a backup — re-enter them after restoring.
backup-restore-from = Backup made { $created } (v{ $version })
backup-restore-settings = Settings
backup-restore-collections = Scene collections ({ $count })
backup-restore-profiles = Profiles ({ $count })
backup-restore-apply = Restore selected
backup-restoring = Restoring…
backup-restored = Restored.
backup-restart-note = Restart Freally Capture to load the restored collections and profiles.
backup-cancel = Cancel
theme-editor-title = Custom palette
theme-color-bg = Background
theme-color-panel = Panel
theme-color-text = Text
theme-color-muted = Muted
theme-color-accent = Accent
theme-color-accent2 = Accent 2
theme-contrast-text-bg = Text on background
theme-contrast-text-panel = Text on panel
theme-contrast-muted-bg = Muted on background
theme-contrast-accent-bg = Accent on background
theme-contrast-low = (aim ≥ { $min }:1)
theme-reset = Reset to dark
theme-export = Export .fctheme…
theme-import = Import .fctheme…
workspace-dock-presets = Dock layouts
workspace-dock-preset-placeholder = Save current layout as…
workspace-dock-preset-save = Save
workspace-dock-preset-apply = Switch to "{ $name }"
workspace-dock-preset-delete = Delete layout "{ $name }"
workspace-dock-presets-hint = A layout remembers the stats dock and mixer orientation. Click one to switch to it.
settings-search-placeholder = Search settings…
settings-search-none = No settings match.
settings-changed = Changed since opened
menu-tools-quick-actions = Quick actions…
menu-tools-featured-chat = Featured Chat Message…
quick-actions-title = Quick actions
quick-actions-empty = No buttons yet — click Edit to add some.
quick-actions-edit = Edit
quick-actions-done = Done
quick-actions-add-page = Page
quick-actions-label = Button label
quick-actions-color = Button colour
quick-actions-add = Add button
quick-actions-remove = Remove "{ $name }"
quick-actions-delete-page = Delete "{ $name }"
quick-actions-hint = Buttons fire the same studio commands as hotkeys and the LAN panel — scenes, macros, and soundboard pads too. Command buttons also show on the LAN panel.
quick-actions-cmd-transition = Transition
quick-actions-cmd-startStream = Go live
quick-actions-cmd-stopStream = End stream
quick-actions-cmd-startRecording = Start recording
quick-actions-cmd-stopRecording = Stop recording
quick-actions-cmd-pauseRecording = Pause recording
quick-actions-cmd-addMarker = Add marker
quick-actions-cmd-armReplay = Arm replay
quick-actions-cmd-saveReplay = Save replay
quick-actions-cmd-setStudioMode = Toggle studio mode
quick-actions-cmd-scene = Switch to scene…
quick-actions-cmd-macro = Run macro…
quick-actions-cmd-soundboard = Soundboard pad…
quick-actions-value-scene = Scene name
quick-actions-value-macro = Macro name
quick-actions-value-soundboard = Pad id
...
history-toggleOutputVisibility = Toggle output visibility
...
palette-featured-chat = Featured chat message…
featured-chat-title = Featured chat message
featured-chat-intro = Pin any recent chat line to the program as a styled bottom banner — viewers of the stream and recording see it. The pin clears on restart; the colors persist.
featured-chat-empty = No chat yet — add a Chat Overlay source and point it at your channel; lines appear here as they arrive.
featured-chat-pinned = On program:
featured-chat-pin = Feature
featured-chat-clear = Clear
featured-chat-bg = Banner color
featured-chat-text = Text color
featured-chat-note = The banner sits bottom-center, above every scene — clear it to take it off air.
...
menu-collection = Scene Collection
menu-tools = Tools
menu-help = Help
menu-rename = Rename
menu-remove = Remove
menu-import = Import
menu-export = Export
menu-file-show-recordings = Show Recordings
menu-file-remux = Remux to MP4…
menu-file-settings = Settings…
menu-file-show-settings-folder = Show Settings Folder
menu-file-exit = Exit
menu-edit-undo = Undo
menu-edit-redo = Redo
menu-edit-history = Edit History…
menu-edit-copy-transform = Copy Transform
menu-edit-paste-transform = Paste Transform
menu-edit-copy-filters = Copy Filters
menu-edit-paste-filters = Paste Filters
menu-edit-transform = Transform…
menu-edit-lock-preview = Lock Preview
menu-view-fullscreen = Fullscreen Interface
menu-stats-dock = Stats Dock
menu-view-multiview = Multiview Monitor…
menu-view-projectors = Projectors…
menu-view-source-health = Source Health…
menu-view-still = Grab Still Frame
menu-docks-browser = Browser Docks…
menu-docks-lock = Lock Docks
menu-docks-reset = Reset Dock Layout
menu-profile-manage = Manage Profiles…
menu-collection-manage = Manage Scene Collections…
menu-collection-import-obs = Import from OBS…
menu-collection-missing = Check for Missing Files…
menu-collection-clear = Clear History…
menu-collection-clear-confirm = Permanently delete these scene collections and their files? { $names } — the active collection "{ $active }" is kept. This cannot be undone.
menu-tools-wizard = Run Setup Wizard
menu-tools-wizard-title = The setup wizard runs on first launch; a re-run path doesn't exist yet.
menu-tools-automation = Automation Rules & Macros…
menu-tools-rundown = Show Rundown…
menu-tools-hotkeys = Hotkey Map…
menu-tools-av-sync = A/V Sync Calibration…
menu-tools-scripts = Lua Scripts…
menu-tools-components = Components…
...
# --- DownstreamKeyers.tsx (CAP-N24) ---
dsk-title = Downstream Keyers
dsk-hint = Overlays composited on the program output — above every scene, and they stay put when you cut scenes (a logo, a LIVE badge, a lower-third). Top of the list draws on top.
dsk-empty = No keyers yet — add a source to overlay it on every scene.
dsk-enable = Enable this keyer
dsk-move-up = Move up (on top)
dsk-move-down = Move down
dsk-remove = Remove keyer
dsk-opacity = Opacity
dsk-x = X (px)
dsk-y = Y (px)
dsk-scale = Scale
...
menu-help-bug = Report a Bug…
menu-help-updates = Check for Updates…
menu-help-whats-new = What's New
...
settings-hotkeys-filter = Filter hotkeys
settings-hotkeys-filter-placeholder = Type to filter actions or keys…
settings-hotkeys-no-match = No hotkey matches “{ $query }”.
settings-hotkey-none = None
settings-hotkey-group-ctrl = Ctrl + key
settings-hotkey-group-ctrl-shift = Ctrl + Shift + key
settings-hotkey-group-ctrl-alt = Ctrl + Alt + key
settings-hotkey-group-function = Function keys
settings-hotkey-group-numpad = Numpad
settings-panic-section = Panic slate
settings-meter-section = Mixer level meters
settings-meter-note = The colors the Audio Mixer's level meters sweep through, from quiet to clipping. The colorblind-safe preset uses a blue → orange ramp that stays readable with red-green color vision deficiency.
settings-meter-preset = Meter colors
settings-meter-preset-default = Green / yellow / red
settings-meter-preset-colorblind = Colorblind-safe (blue / orange)
settings-meter-preset-custom = Custom
settings-meter-low = Normal
settings-meter-mid = Loud
settings-meter-high = Clipping
settings-meter-preview = Preview

# --- Telestrator (CAP-N57) ---
telestrator-group = Telestrator
telestrator-draw = Draw
telestrator-tool-pen = Pen
telestrator-tool-highlight = Highlighter
telestrator-tool-arrow = Arrow
telestrator-tool-ellipse = Ellipse
telestrator-color = Color
telestrator-width = Width
telestrator-whiteboard = Whiteboard mode
telestrator-persist = Keep
telestrator-fade = Fade
telestrator-undo = Undo last mark
telestrator-clear = Clear marks
hotkeys-telestrator-clear = Telestrator: clear marks

# --- Teleprompter (CAP-N58) ---
teleprompter-title = Teleprompter
teleprompter-loading = Loading…
teleprompter-empty = No script yet — add one in the teleprompter panel.
teleprompter-script = Script
teleprompter-script-placeholder = Type or paste your script…
teleprompter-top = Top
teleprompter-slower = Slower
teleprompter-play = Play
teleprompter-pause = Pause
teleprompter-stop = Stop
teleprompter-faster = Faster
teleprompter-step-back = Step back
teleprompter-step-forward = Step forward
teleprompter-est-time = Read time
teleprompter-caesura-hint = Type -- for a pause
teleprompter-caesura-pause = Caesura — type "--" to add a pause
teleprompter-countdown = Start countdown
teleprompter-countdown-secs = Countdown (seconds)
teleprompter-speed = Speed (chars/sec)
teleprompter-speed-bpm = Speed (BPM)
teleprompter-bpm-mode = BPM mode (singing)
teleprompter-font = Font size
teleprompter-mirror = Mirror (glass)
teleprompter-open-projector = Open projector
teleprompter-preview = Preview
teleprompter-remote-hint = Control the scroll from a hotkey, MIDI, or the LAN panel while this is closed.
teleprompter-read-aloud = Read aloud with per-OS speech synthesis
menu-tools-teleprompter = Teleprompter
hotkeys-teleprompter-toggle = Teleprompter: play / pause

# --- Remote guests: green room / cues / QoS / auto-grid (CAP-N54–N56, N59) ---
remote-hosting-count = Hosting { $count } guest(s)
remote-green-room-default = Green room
remote-green-room-default-title = New guests wait in the green room until you seat them
remote-auto-grid = Auto-grid
remote-auto-grid-title = Keep guests arranged in a grid as they join and leave
remote-arrange-grid = Arrange grid
remote-arrange-grid-title = Reflow the seated guests into a grid now
remote-green-room-monitor = Green-room preview
remote-tech-cam-ok = Cam ✓
remote-tech-cam-no = No cam
remote-tech-mic-ok = Mic ✓
remote-tech-mic-no = No mic
remote-seat-on-air = Seat on air
remote-cues-label = Cues
remote-cue = Cue
remote-cue-thirty = 30 seconds
remote-cue-wrap = Wrap up
remote-cue-next = You're next
remote-cue-speak = Speak up
remote-qos-good = Connection good
remote-qos-fair = Connection fair
remote-qos-poor = Connection poor
remote-green-room-guest = You're in the green room — the host will put you on air shortly.

# --- Hotkey audit — Phase 7 actions ---
hotkey-audit-action-telestrator-clear = Telestrator: clear marks
hotkey-audit-action-teleprompter-toggle = Teleprompter: play / pause
...
sources-add-starting-soon = Starting Soon…
sources-starting-soon-title = Starting Soon
sources-starting-soon-default = Starting Soon
sources-starting-soon-message = Message
sources-starting-soon-minutes = Minutes
sources-starting-soon-hours = Hours
sources-starting-soon-start-at = Start at
sources-starting-soon-music = Background music…
sources-starting-soon-music-name = Background Music
sources-starting-soon-background = Background
sources-starting-soon-note = A full-screen countdown that flashes red and holds at zero — clear it and cut to your live scene by hand when you're ready.
sources-starting-soon-add = Add
sources-slate-solid = Solid color
sources-slate-gradient = Gradient
sources-slate-transparent = Transparent
sources-slate-image = Image
sources-slate-file = Image / Video / GIF
sources-slate-browse = Browse…
sources-starting-soon-choose-image = Choose image…
sources-slate-media-note = Still images embed here. For a looping video or GIF background, choose Transparent and set the scene's Backdrop (🖼 in the Scenes rail) — the countdown sits on top.
properties-timer-slate = Starting Soon slate
properties-timer-slate-off = Off (inline text)
properties-timer-message = Message
properties-timer-slate-color = Slate color
properties-timer-slate-from = From
properties-timer-slate-to = To
properties-timer-slate-note = Transparent lets an image, video or scene backdrop beneath the slate show through.
