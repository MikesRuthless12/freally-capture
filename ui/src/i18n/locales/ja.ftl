...
sources-add-background-music = BGM…
sources-background-music-note = ループ再生され、プレビューで聞こえ、録画/配信にも入ります。動画の音の代わりに音楽を使うには、ミキサーで動画の Backdrop ストリップをミュートしてください。
sources-background-music-count = { $n } 曲
sources-background-music-shuffle = シャッフル
...
sources-stream-hide = 配信で非表示
sources-stream-show = 配信で表示
sources-stream-hide-item = { $name } を配信で非表示
sources-stream-show-item = { $name } を配信で表示
sources-record-hide = 録画で非表示
sources-record-show = 録画で表示
sources-record-hide-item = { $name } を録画で非表示
sources-record-show-item = { $name } を録画で表示
...
controls-iso-lanes = 番組と並行して録画中の ISO レーン: { $count }
...
ltc-badge = LTC
ltc-title = SMPTE タイムコード（LTC）
ltc-intro = トラックに SMPTE リニアタイムコードを生成し、任意の音声入力から入力 LTC を読み取ります — 外部レコーダーやカメラをポストで同期する定番の音声タイムコードです。完全オフライン。
ltc-generate = トラックに LTC を生成
ltc-track = タイムコードトラック
ltc-track-option = トラック { $track }
ltc-fps = フレームレート
ltc-read = LTC の読み取り元
ltc-read-off = オフ
ltc-decoded = 入力タイムコード
ltc-no-lock = 信号なし
ltc-note = ジェネレーターは時刻にジャムシンクします（ノンドロップ）。そのトラックを録音する（出力設定で割り当て）か、出力へルーティングして外部機器へ送ります。リーダーは統計オーバーレイのタイムコード行を駆動し、チャプターマーカーに刻印します。
...
stats-disk = ディスク
stats-disk-free = 空き
stats-disk-left = 録画残り
stats-disk-rate = ≈ { $rate } MB/s 録画
stats-vertical-fps = 9:16 FPS
stats-targets-label = 配信ターゲット
stats-rehearsal-note = リハーサル — 配信先はローカルシンクのみ
stats-timeline-open = タイムライン
timeline-title = セッションタイムライン
timeline-empty = まだ記録がありません — 配信または録画中にタイムラインが記録されます。
timeline-live = LIVE — 記録中
timeline-fit = 全体表示
timeline-legend-fps = fps
timeline-legend-behind = エンコーダー待ち（遅延フレーム）
...
scenes-recent-title = 最近
scenes-recent-clear = クリア
scenes-recent-clear-confirm = 最近のシーン一覧をクリアしますか？
...
livebutton-rehearse = リハーサル
livebutton-rehearse-record-confirm = このリハーサルも動画ファイルに録画しますか？
livebutton-rehearse-title = ショー全体をローカルシンクに対して実行 — 何も送信されません
livebutton-end-rehearsal = リハーサルを終了
livebutton-title-rehearsing = リハーサル実行中 — このマシンから何も出ていきません
livebutton-badge-rehearsal = リハーサル
livebutton-aria-rehearsal = リハーサル中
livebutton-rehearsal-banner = リハーサル — このマシンから何も出ていきません
...
properties-timer-clock = 指定時刻までカウントダウン
...
filters-name-perspective = パース
filters-name-fade-loop = フェードループ
...
filters-tilt = 傾き
filters-far-fade = 遠端フェード
filters-fade-in-s = フェードイン（秒）
filters-visible-s = 表示（秒）
filters-fade-out-s = フェードアウト（秒）
filters-hidden-s = 非表示（秒）
filters-crop-left = 左
filters-crop-top = 上
...
recordings-trim = トリム
recordings-trim-title = この録画からクリップを切り出します — キーフレームに揃えたカットは再エンコードなしで書き出されます
recordings-verify = 検証
recordings-verify-title = ファイルの整合性を確認 — コンテナ構造、連続性、A/V インターリーブ、長さ
recordings-verifying = 検証中…
verify-dismiss = 閉じる
verify-verdict-pass = { $name } — 整合性に問題なし
verify-verdict-warn = { $name } — 警告付きで検証済み
verify-verdict-fail = { $name } — 問題が見つかりました
verify-container = コンテナ
verify-video-continuity = 映像の連続性
verify-audio-continuity = 音声の連続性
verify-av-interleave = A/V インターリーブ
verify-duration = 長さ
recordings-alpha-label = アルファ
recordings-prores-title = アルファを保持した ProRes 4444 .mov マスターを書き出す（編集用）
recordings-qtrle-title = アルファを保持した QuickTime Animation .mov を書き出す（最大互換）
trim-title = トリム — { $name }
trim-loading = ファイルを読み込み中…
trim-preview-alt = プレビューフレーム
trim-position = 再生位置
trim-step-second-back = 1 秒戻る
trim-step-frame-back = 1 フレーム戻る
trim-step-frame-forward = 1 フレーム進む
trim-step-second-forward = 1 秒進む
trim-snap = キーフレーム
trim-snap-title = 最寄りのキーフレームへスナップ — その位置のカットは再エンコードなしで書き出せます
trim-set-in = イン点を設定
trim-set-out = アウト点を設定
trim-range-invalid = アウト点はイン点より後にしてください。
trim-copy-badge = ✓ 再エンコードなしで書き出します — イン点がキーフレーム上にあります。
trim-reencode-badge = 再エンコードされます: イン点がキーフレームの間にあります（「キーフレーム」でスナップすればロスレスカット）。
trim-export = クリップを書き出す
trim-export-916 = 9:16
trim-export-916-title = 縦型にリフレームして書き出し（縦キャンバスのサイズで中央クロップ）— 常に再エンコードします
...
bench-open = エンコーダーベンチマークを実行…
bench-title = エンコーダーベンチマーク
bench-intro = このマシンで短い実測エンコードラダー（検出された全エンコーダー × プリセット × 解像度）を実行します — 約1分、完全オフラインで、何も外に送信されません。失敗は隠さず一覧表示。先に配信・録画を停止してください。
bench-start = ベンチマーク開始
bench-rerun = 再実行
bench-running = 計測中… { $done } / { $total }
bench-cancel = キャンセル
bench-col-encoder = エンコーダー
bench-col-preset = プリセット
bench-col-rung = 段
bench-col-achieved = fps
bench-col-headroom = 余裕
bench-failed = 失敗
bench-rec-title = 推奨（実測）
bench-rec-body = { $encoder }（{ $preset }）、{ $width }×{ $height } @ { $fps } fps — 実測 { $headroom }× リアルタイム。推奨配信ビットレート: { $bitrate } kbps。
bench-rec-none = このマシンではリアルタイムを維持できる設定がありません — キャンバス解像度か fps を下げて再計測してください。
bench-apply = 録画設定に適用
bench-applied = 適用済み ✓
...
output-iso-heading = ISO 録画
output-iso-explainer = 選択したソースをクリーンな状態で、番組と並行してそれぞれ独自のファイルに録画します — 合成前、キャンバスのサイズとフレームレートで記録されるため、各ファイルは編集タイムラインにそのまま揃って並びます。ミッドレンジ GPU ならレーン 2 本まで快適です。レーンを増やすごとにレンダリングとエンコードが 1 回ずつ増えます。
output-iso-none = コレクションにはまだソースがありません。
output-iso-source-on = 「{ $name }」を ISO ファイルとして録画中 — クリックで停止
output-iso-source-off = 「{ $name }」を独自の ISO ファイルとして録画
output-iso-post-filter = ソースのフィルターを適用して録画（ポストフィルター）。オフにすると未加工のソースを録画します
output-iso-format = ISO フォーマット
output-iso-encoder = ISO 映像エンコーダー
output-alpha-frec = 透過（アルファ）付きで録画 — 透明背景の上に番組を記録
output-auto-export = マスターの隣に共有用MP4も保存
output-auto-export-title = .frec録画の確定時に、バックグラウンドでダブルクリック再生できるMP4コピーを隣に書き出します（バックグラウンドキューに表示）。ロスレスのマスターはそのまま残ります。
output-alpha-title = レコーダーは専用の透過レンダリングを受け取ります。プレビューと配信は通常のままです。録画リストから ProRes 4444 か QTRLE に書き出せます — MP4/MKV はアルファを潰します。
output-split-events = 次のタイミングでも新しいファイルを開始… (各パートはイベントちょうどで始まります。最小長 1 秒)
output-split-on-scene = シーン切り替え
output-split-on-marker = マーカー
output-split-on-rundown = ランダウンのステップ
output-auto-markers = スタジオイベントで自動的にチャプターマーカーを打つ（シーン切替、リプレイ保存、再接続、フレーム落ち、アラーム、ルール）
output-auto-markers-title = 型付きマーカーは録画のチャプター（mkv）または .chapters.txt サイドカーに、手動マーカーのホットキーと並んで記録されます
output-pipeline-heading = 録画後パイプライン
output-pipeline-explainer = 録画の確定後、メインファイルに対してこれらのステップを順にバックグラウンドで実行します。閉じたアクションセットです — 「コマンドを実行」ステップは意図的にありません。チェーンは最初の失敗で停止します。
output-pipeline-enabled = 毎回の録画後にパイプラインを実行
output-pipeline-add = ステップを追加…
output-pipeline-up = 上へ
output-pipeline-down = 下へ
output-pipeline-remove = ステップを削除
output-pipeline-template = リネームテンプレート（CAP-M25 トークン）
output-pipeline-folder = フォルダー
pipeline-queue = 録画後パイプライン
pipeline-verify = 検証
pipeline-remux = MP4 へリマックス
pipeline-normalize = ラウドネス正規化
pipeline-rename = リネーム
pipeline-move = フォルダーへ移動
pipeline-copy = フォルダーへコピー
pipeline-reveal = ファイルマネージャーで表示
pipeline-luaEvent = Lua スクリプトへ通知
...
stream-session-report = セッション終了時に録画の隣へレポート（HTML + Markdown）を書き出す
stream-simulator-title = ネットワークシミュレーター（リハーサル）
stream-simulator-note = リハーサルのローカルシンクだけを制限 — 再接続や細い回線を練習。本番の配信には一切影響しません。
stream-simulator-profile = プロファイル
stream-simulator-off = オフ
stream-simulator-hotel-wifi = ホテルWi-Fi
stream-simulator-mobile-hotspot = モバイルホットスポット
stream-simulator-custom = カスタム
stream-simulator-bandwidth = 帯域（kbps、0 = 無制限）
stream-simulator-latency = 遅延（ms）
stream-simulator-jitter = ジッター（± ms）
stream-simulator-outage-every = 切断間隔（秒、0 = なし）
stream-simulator-outage-len = 切断時間（秒）
...
workspace-pack-title = シーンコレクションパック
workspace-pack-export = パックをエクスポート…
workspace-pack-exporting = エクスポート中…
workspace-pack-import = パックをインポート…
workspace-pack-importing = インポート中…
workspace-pack-hint = .fcappack は、アクティブなコレクションとそのローカルアセットを 1 つのポータブルファイルにまとめます — レイアウト全体をサーバーなしで共有できます。インポートすると新しいコレクションとして取り込み、そこへ切り替えます。
workspace-pack-exported = 「{ $name }」をエクスポートしました — アセット { $bundled } 個をバンドル、{ $external } 個は外部。
workspace-pack-imported = 「{ $name }」をインポートしました — シーン { $scenes } 個、アセット { $relinked } 個を再リンク、{ $external } 個は外部。
workspace-compare = 比較してマージ…
compare-title = 比較してマージ
compare-intro = アクティブなコレクションを別のものと比較し、選んだ変更だけを取り込みます。何もこのマシンから出ません。
compare-target-label = 比較対象
compare-target-none = コレクションを選択…
compare-target-pack = パックファイルから…
compare-against = { $name } と比較
compare-comparing = 比較中…
compare-no-diff = 差分なし — コレクションは一致しています。
compare-select-all = すべて選択
compare-clear = クリア
compare-merge-hint = シーンを選ぶと、それが使うソースも一緒に取り込まれます。
compare-sources-title = ソース
compare-scenes-title = シーン
compare-kind-added = 追加
compare-kind-removed = 削除
compare-kind-modified = 変更
compare-renamed = 以前は「{ $from }」
compare-reordered = 並べ替え
compare-apply = { $count } 件を適用
compare-apply-none = 適用する変更を選択
compare-applying = 適用中…
compare-aspect-source = ソース
compare-aspect-transform = 位置
compare-aspect-visibility = 表示
compare-aspect-outputs = 出力
compare-aspect-filters = フィルタ
compare-aspect-lock = ロック
compare-aspect-blend = ブレンド
compare-aspect-scaling = スケーリング
compare-aspect-backdrop = 背景
compare-aspect-reveal = リビール
compare-aspect-name = 名前
compare-aspect-settings = 設定
compare-aspect-audio = 音声
workspace-snapshots = スナップショット…
snapshots-title = スナップショット
snapshots-intro = 戻れる、このコレクションの名前付きチェックポイント — 自動保存や元に戻すとは別に、意図的に保持されます。ローカルに保存。
snapshots-name-placeholder = このチェックポイントに名前を付ける…
snapshots-create = スナップショットを保存
snapshots-empty = まだスナップショットがありません — 保存して、戻れる名前付きチェックポイントを残しましょう。
snapshots-usage = { $cap } 個中 { $count } 個を保持 · { $size }
snapshots-scenes = シーン { $scenes } · ソース { $sources }
snapshots-restore = 復元
snapshots-compare = 比較
snapshots-delete = スナップショットを削除
snapshots-delete-aria = スナップショット「{ $name }」を削除
snapshots-restore-confirm = このスナップショットを復元しますか？現在のレイアウトが先にスナップショットとして保存されます。

# --- Phase 8 (backup / theme editor / dock presets / settings search / quick actions) ---
workspace-backup = バックアップと復元…
backup-title = バックアップと復元
backup-intro = スタジオ全体（設定・プロファイル・シーンコレクション）を1つのファイルに保存し、ここや別のマシンで復元できます。
backup-export = バックアップをエクスポート…
backup-exporting = エクスポート中…
backup-exported = コレクション { $collections } 個とプロファイル { $profiles } 個をバックアップしました（{ $size }）。
backup-restore = バックアップから復元…
backup-secrets-note = ストリームキーとパスワードはバックアップに含まれません — 復元後に再入力してください。
backup-restore-from = { $created } に作成されたバックアップ（v{ $version }）
backup-restore-settings = 設定
backup-restore-collections = シーンコレクション（{ $count }）
backup-restore-profiles = プロファイル（{ $count }）
backup-restore-apply = 選択項目を復元
backup-restoring = 復元中…
backup-restored = 復元しました。
backup-restart-note = 復元したコレクションとプロファイルを読み込むには Freally Capture を再起動してください。
backup-cancel = キャンセル
theme-editor-title = カスタムパレット
theme-color-bg = 背景
theme-color-panel = パネル
theme-color-text = テキスト
theme-color-muted = サブテキスト
theme-color-accent = アクセント
theme-color-accent2 = アクセント 2
theme-contrast-text-bg = 背景上のテキスト
theme-contrast-text-panel = パネル上のテキスト
theme-contrast-muted-bg = 背景上のサブテキスト
theme-contrast-accent-bg = 背景上のアクセント
theme-contrast-low = （目標 ≥ { $min }:1）
theme-reset = ダークにリセット
theme-export = .fctheme をエクスポート…
theme-import = .fctheme をインポート…
workspace-dock-presets = ドックレイアウト
workspace-dock-preset-placeholder = 現在のレイアウトを保存…
workspace-dock-preset-save = 保存
workspace-dock-preset-apply = 「{ $name }」に切り替え
workspace-dock-preset-delete = レイアウト「{ $name }」を削除
workspace-dock-presets-hint = レイアウトは統計ドックとミキサーの向きを記憶します。クリックすると切り替わります。
settings-search-placeholder = 設定を検索…
settings-search-none = 一致する設定はありません。
settings-changed = 開いてから変更あり
menu-tools-quick-actions = クイックアクション…
menu-tools-featured-chat = 注目チャットメッセージ…
quick-actions-title = クイックアクション
quick-actions-empty = ボタンがまだありません — 編集をクリックして追加してください。
quick-actions-edit = 編集
quick-actions-done = 完了
quick-actions-add-page = ページ
quick-actions-label = ボタンのラベル
quick-actions-color = ボタンの色
quick-actions-add = ボタンを追加
quick-actions-remove = 「{ $name }」を削除
quick-actions-delete-page = 「{ $name }」を削除
quick-actions-hint = ボタンはホットキーや LAN パネルと同じスタジオコマンド（シーン・マクロ・サウンドボードパッドを含む）を実行します。コマンドボタンは LAN パネルにも表示されます。
quick-actions-cmd-transition = トランジション
quick-actions-cmd-startStream = 配信開始
quick-actions-cmd-stopStream = 配信を終了
quick-actions-cmd-startRecording = 録画開始
quick-actions-cmd-stopRecording = 録画停止
quick-actions-cmd-pauseRecording = 録画を一時停止
quick-actions-cmd-addMarker = マーカーを追加
quick-actions-cmd-armReplay = リプレイを準備
quick-actions-cmd-saveReplay = リプレイを保存
quick-actions-cmd-setStudioMode = スタジオモード切替
quick-actions-cmd-scene = シーンに切り替え…
quick-actions-cmd-macro = マクロを実行…
quick-actions-cmd-soundboard = サウンドボードパッド…
quick-actions-value-scene = シーン名
quick-actions-value-macro = マクロ名
quick-actions-value-soundboard = パッド ID
teleprompter-stop = 停止
about-portable-on = ポータブルモード: オン — すべてのアプリデータはアプリの隣に保存されます。
about-portable-off = ポータブルモード: オフ。アプリを完全に自己完結で実行するには、アプリの隣に「{ $marker }」ファイルを追加します（USB メモリモード）。
about-portable-data = データフォルダー: { $path }
...
history-toggleOutputVisibility = 出力ごとの表示を切り替え
...
palette-featured-chat = 注目チャットメッセージ…
featured-chat-title = 注目チャットメッセージ
featured-chat-intro = 最近のチャット行をプログラム下部のバナーとしてピン留めします。配信や録画の視聴者にも表示されます。ピンは再起動で消え、色は保存されます。
featured-chat-empty = まだチャットがありません。チャットオーバーレイソースを追加してチャンネルに接続すると、届いた行がここに表示されます。
featured-chat-pinned = 番組表示中:
featured-chat-pin = 表示
featured-chat-clear = 解除
featured-chat-bg = バナー色
featured-chat-text = 文字色
featured-chat-note = バナーは全シーンの上、下部中央に表示されます。解除すると放送から消えます。
health-title = ソースの状態
...
properties-stats-show-timecode = タイムコードを表示（LTC）
...
sources-visualizer-classic = クラシックカラー
...
menu-bar-label = アプリケーションメニュー
menu-file = ファイル
menu-edit = 編集
menu-view = 表示
menu-docks = ドック
menu-profile = プロファイル
menu-collection = シーンコレクション
menu-tools = ツール
menu-help = ヘルプ
menu-rename = 名前を変更
menu-remove = 削除
menu-import = インポート
menu-export = エクスポート
menu-file-show-recordings = 録画を表示
menu-file-remux = MP4にRemux…
menu-file-settings = 設定…
menu-file-show-settings-folder = 設定フォルダーを表示
menu-file-exit = 終了
menu-edit-undo = 元に戻す
menu-edit-redo = やり直す
menu-edit-history = 編集履歴…
menu-edit-copy-transform = 変形をコピー
menu-edit-paste-transform = 変形を貼り付け
menu-edit-copy-filters = フィルターをコピー
menu-edit-paste-filters = フィルターを貼り付け
menu-edit-transform = 変形…
menu-edit-lock-preview = プレビューをロック
menu-view-fullscreen = フルスクリーンインターフェイス
menu-stats-dock = 統計パネル
menu-view-multiview = マルチビューモニター…
menu-view-projectors = プロジェクター…
menu-view-source-health = ソースの状態…
menu-view-still = 静止画をキャプチャ
menu-docks-browser = ブラウザードック…
menu-docks-lock = ドックをロック
menu-docks-reset = ドック配置をリセット
menu-profile-manage = プロファイルを管理…
menu-collection-manage = シーンコレクションを管理…
menu-collection-import-obs = OBS からインポート…
menu-collection-missing = 不明なファイルを確認…
menu-collection-clear = 履歴をクリア…
menu-collection-clear-confirm = これらのシーンコレクションとそのファイルを完全に削除しますか？ { $names } — アクティブなコレクション「{ $active }」は保持されます。この操作は元に戻せません。
menu-tools-wizard = セットアップウィザードを実行
menu-tools-wizard-title = セットアップウィザードは初回起動時に実行されます。再実行する方法はまだありません。
menu-tools-automation = オートメーションのルールとマクロ…
menu-tools-rundown = 番組進行表を表示…
menu-tools-hotkeys = ホットキー一覧…
menu-tools-av-sync = A/V 同期キャリブレーション…
menu-tools-scripts = Lua スクリプト…
menu-tools-components = コンポーネント…
menu-tools-midi = MIDI コントロール…
menu-tools-ptz = PTZ カメラ…
menu-tools-remote = リモート制御 API…
menu-tools-panel = LANパネルとタリー…
menu-help-portal = ヘルプポータル
menu-help-website = ウェブサイトを開く
menu-help-discord = Discord サーバーに参加
menu-help-bug = バグを報告…
...
settings-cat-streaming = 配信
settings-cat-output = 出力
settings-cat-replay = リプレイ
settings-cat-hotkeys = ホットキー
settings-cat-network = ネットワーク
settings-cat-accessibility = アクセシビリティ
settings-cat-about = バージョン情報
settings-ok = OK
settings-cancel = キャンセル
settings-apply = 適用
settings-save = 保存
settings-loading = 設定を読み込み中…
settings-hotkeys-filter = ホットキーを絞り込む
settings-hotkeys-filter-placeholder = 入力して操作やキーを絞り込み…
settings-hotkeys-no-match = “{ $query }” に一致するホットキーはありません。
settings-hotkey-none = なし
settings-hotkey-group-ctrl = Ctrl + キー
settings-hotkey-group-ctrl-shift = Ctrl + Shift + キー
settings-hotkey-group-ctrl-alt = Ctrl + Alt + キー
settings-hotkey-group-function = ファンクションキー
settings-hotkey-group-numpad = テンキー
settings-panic-section = パニックスレート
settings-meter-section = ミキサーのレベルメーター
settings-meter-note = 音声ミキサーのレベルメーターが静かな状態からクリッピングまで変化していく色です。色覚バリアフリーのプリセットは、赤緑色覚異常でも判別できる青→オレンジのグラデーションを使います。
settings-meter-preset = メーターの色
settings-meter-preset-default = 緑 / 黄 / 赤
settings-meter-preset-colorblind = 色覚バリアフリー（青 / オレンジ）
settings-meter-preset-custom = カスタム
settings-meter-low = 通常
settings-meter-mid = 大音量
settings-meter-high = クリッピング
settings-meter-preview = プレビュー

# --- CAP-N: What's New, blur/pixelate/freeze filters, 3D transform, clone, Downstream Keyers ---
whats-new-title = 新機能
whats-new-loading = リリースノートを読み込み中…
whats-new-version = バージョン { $version } の新機能
whats-new-empty = このバージョンのリリースノートはありません。
filters-name-directional-blur = 方向ブラー
filters-name-radial-blur = 放射ブラー
filters-name-zoom-blur = ズームブラー
filters-name-pixelate = モザイク
filters-angle = 角度 (°)
filters-center-x = 中心 X
filters-center-y = 中心 Y
filters-block-size = ブロックサイズ (px)
filters-name-freeze = フリーズ
filters-freeze-hint = 有効にすると、このソースは最後のフレームを保持します — プログラム、プレビュー、録画、配信がすべて同時にフリーズします。このフィルターを切り替えてフリーズまたは解除します。
transform-3d = 3D チルト
...
# --- Telestrator (CAP-N57) ---
telestrator-group = テレストレーター
telestrator-draw = 描画
telestrator-tool-pen = ペン
telestrator-tool-highlight = マーカー
telestrator-tool-arrow = 矢印
telestrator-tool-ellipse = 楕円
telestrator-color = 色
telestrator-width = 太さ
telestrator-whiteboard = ホワイトボードモード
telestrator-persist = 保持
telestrator-fade = フェード
telestrator-undo = 最後の書き込みを元に戻す
telestrator-clear = 書き込みを消去
hotkeys-telestrator-clear = テレストレーター: 書き込みを消去

# --- Teleprompter (CAP-N58) ---
teleprompter-title = テレプロンプター
teleprompter-loading = 読み込み中…
teleprompter-empty = 台本がありません — テレプロンプターパネルで追加してください。
teleprompter-script = 台本
teleprompter-script-placeholder = 台本を入力または貼り付け…
teleprompter-top = 先頭
teleprompter-slower = 遅く
teleprompter-play = 再生
teleprompter-pause = 一時停止
teleprompter-faster = 速く
teleprompter-step-back = 少し戻る
teleprompter-step-forward = 少し進む
teleprompter-est-time = 読み上げ時間
teleprompter-caesura-hint = 一時停止するには -- と入力
teleprompter-caesura-pause = 一時停止 — 一時停止するには -- と入力
teleprompter-countdown = カウントダウン開始
teleprompter-countdown-secs = カウントダウン（秒）
teleprompter-speed = 速度（文字/秒）
teleprompter-speed-bpm = 速度（BPM）
teleprompter-bpm-mode = BPM モード（歌唱）
teleprompter-font = 文字サイズ
teleprompter-mirror = 反転（ガラス）
teleprompter-open-projector = プロジェクターを開く
teleprompter-preview = プレビュー
teleprompter-remote-hint = これを閉じている間は、ホットキー・MIDI・LAN パネルでスクロールを操作できます。
teleprompter-read-aloud = OS 標準の音声合成で読み上げ
menu-tools-teleprompter = テレプロンプター
hotkeys-teleprompter-toggle = テレプロンプター: 再生 / 一時停止

# --- Remote guests: green room / cues / QoS / auto-grid (CAP-N54–N56, N59) ---
remote-hosting-count = { $count } 人のゲストをホスト中
remote-green-room-default = グリーンルーム
remote-green-room-default-title = 新しいゲストは、席に着けるまでグリーンルームで待機します
remote-auto-grid = 自動グリッド
remote-auto-grid-title = ゲストの入退室に合わせてグリッドを保つ
remote-arrange-grid = グリッド整列
remote-arrange-grid-title = 出演中のゲストを今すぐグリッドに並べ直す
remote-green-room-monitor = グリーンルームのプレビュー
remote-tech-cam-ok = カメラ ✓
remote-tech-cam-no = カメラなし
remote-tech-mic-ok = マイク ✓
remote-tech-mic-no = マイクなし
remote-seat-on-air = オンエアにする
remote-cues-label = キュー
remote-cue = キュー
remote-cue-thirty = 残り30秒
remote-cue-wrap = そろそろまとめて
remote-cue-next = 次はあなたです
remote-cue-speak = もっと大きな声で
remote-qos-good = 接続良好
remote-qos-fair = 接続やや不安定
remote-qos-poor = 接続不良
remote-green-room-guest = グリーンルームで待機中です — まもなくホストがオンエアにします。

# --- Hotkey audit — Phase 7 actions ---
hotkey-audit-action-telestrator-clear = テレストレーター: 書き込みを消去
hotkey-audit-action-teleprompter-toggle = テレプロンプター: 再生 / 一時停止
hotkey-audit-feature-telestrator = テレストレーター
hotkey-audit-feature-teleprompter = テレプロンプター

# V1-C — Starting Soon countdown slate
sources-add-starting-soon = まもなく開始…
sources-starting-soon-title = まもなく開始
sources-starting-soon-default = まもなく開始
sources-starting-soon-message = メッセージ
sources-starting-soon-minutes = 分
sources-starting-soon-hours = 時間
sources-starting-soon-start-at = 開始時刻
sources-starting-soon-music = BGM…
sources-starting-soon-music-name = BGM
sources-starting-soon-background = 背景
sources-starting-soon-note = 全画面のカウントダウン。ゼロで赤く点滅して停止します。準備ができたら手動でクリアし、ライブシーンに切り替えてください。
sources-starting-soon-add = 追加
sources-slate-solid = 単色
sources-slate-gradient = グラデーション
sources-slate-transparent = 透明
sources-slate-image = 画像
sources-slate-file = 画像 / 動画 / GIF
sources-slate-browse = 参照…
sources-starting-soon-choose-image = 画像を選択…
sources-slate-media-note = 静止画はここに埋め込まれます。ループ動画やGIFを背景にするには「透明」を選び、シーンの背景（シーンレールの🖼）を設定してください。カウントダウンはその上に表示されます。
properties-timer-slate = 「まもなく開始」スレート
properties-timer-slate-off = オフ（インラインテキスト）
properties-timer-message = メッセージ
properties-timer-slate-color = スレートの色
properties-timer-slate-from = 開始色
properties-timer-slate-to = 終了色
properties-timer-slate-note = 透明にすると、スレートの下にある画像・動画・シーン背景が透けて見えます。
...
sources-badge-browser = ブラウザ
sources-add-browser = ブラウザ（ウェブページ）
sources-browser-title = ブラウザソースを追加
sources-browser-add = ブラウザソースを追加
sources-browser-url = ページ URL（http:// または https://）
sources-browser-url-invalid = http:// または https:// の URL を入力してください — ローカルファイルはメディア／画像ソースで再生します。
sources-browser-width = 幅 (px)
sources-browser-height = 高さ (px)
