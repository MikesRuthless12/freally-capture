...
/**
 * The active locale, held outside React so non-component code (toasts, the
 * native-preview bridge, error paths) can translate without a hook. A tiny
 * hand-rolled store, matching `lib/modal.ts` — the app has no state library and
 * this does not justify introducing one.
 */
...
/**
 * Switch language. Also stamps `<html lang>` and `<html dir>`, which is what
 * actually flips the layout for Arabic — CSS logical properties and the
 * browser's own bidi algorithm key off `dir`, not off a React prop.
 */
...
/** Test seam: forget the active locale between cases. */
