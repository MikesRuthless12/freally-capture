/**
 * Teleprompter "read aloud" pace preview.
 *
 * Speaks the script at the pace the scroll is set to, and reports progress so the
 * modal preview can highlight the word being spoken. Preview only — never the
 ...
 * gap between them, so `--` is never vocalized and each pause lasts its duration.
 *
 * Backend is a hybrid: the WebView's `speechSynthesis` (native Windows
 * OneCore/SAPI + macOS AVSpeechSynthesis) is used wherever it has voices; on
 * Linux (WebKitGTK usually ships none) it falls back to the native `tts_speak`
 * command, which shells out to the user's Speech Dispatcher / espeak-ng.
 *
...
// fencing spaces) or 0. A caesura next to a line break used to be read as "dash
// dash" because the old check demanded a literal space on each side.
...
export async function readAloud(
  ...
  const playNext = () => {
    ...
    session.native = true;
    void ttsSpeakNative(chunk.text, rate);
