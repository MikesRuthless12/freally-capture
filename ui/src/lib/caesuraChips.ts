// Pure helpers for the caesura CHIP editor (CaesuraEditor.tsx): splitting a
// script string into text runs and ` -- ` / ` --N ` caesura tokens, labelling a
// chip, and normalizing pasted text. Kept DOM-free and separate from the
// component so it's unit-testable and liftable into the standalone Freally
// Teleprompt clone. The fence rules mirror the Rust `parse_caesuras` and the TS
// `parseCaesuras` (in ./caesura) exactly, so the chips a user sees are precisely
// what the scroll treats as pauses.
