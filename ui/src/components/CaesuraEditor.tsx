...
import { loadDict, suggest, type LoadedDict } from "../lib/suggest";

// A contenteditable script editor where every inline ` -- ` / ` --N ` caesura is
// rendered as an atomic, non-editable CHIP. The caret can only land before or
// after a chip (never inside it), Backspace/Delete removes the whole token in one
// go, Shift+Arrow selects it as a unit, and copy/cut carry the token text — all
// the "atomic" behaviour comes natively from `contenteditable="false"` on the
// chip, with a few surgical key handlers to match the operator's expectations.
//
...
// tokenizer lives in ../lib/caesuraChips so it can be unit-tested and lifted into
// the standalone Freally Teleprompt clone.
 ...
 * (accent pill) so the component stays self-contained and theme-aware. */
    ...
    node.childNodes.forEach((child) => {
      if (child.nodeType === Node.TEXT_NODE) {
        ...
        if (el.dataset.ghost !== undefined) {
          // Transient autocomplete ghost — never part of the script.
...
function nodeLen(node: Node): number {
  ...
  if (el.dataset && el.dataset.ghost !== undefined) return 0;
...
function offsetOf(root: HTMLElement, container: Node, offsetInNode: number): number {
  ...
  const rec = (node: Node) => {
    ...
    const el = node as HTMLElement;
    if (el.dataset && el.dataset.ghost !== undefined) return;
...
export function CaesuraEditor({
  ...
  /** The script string (server-authoritative; external changes rebuild the DOM). */
  value: string;
  ...
  /** Active locale for ghost-text autocomplete; omit to disable suggestions. */
  lang?: string;
}) {
  ...
  // External value change (initial mount, script loaded/replaced elsewhere):
  ...
  // Load the active language's suggestion dictionary (lazy, cached).
  const dictRef = useRef<LoadedDict | null>(null);
  ...
  // Ghost-text autocomplete: a light-grey, non-editable suffix after the caret;
  // Tab accepts it, any other key / click / blur dismisses it. It carries
  // `data-ghost`, which serialize()/nodeLen()/offsetOf() ignore, so it never
  // enters the script or the caret math.
  const removeGhost = () => {
  ...
  const showGhost = (suffix: string) => {
    ...
    span.style.cssText = "opacity:0.45;pointer-events:none;user-select:none;";
    range.insertNode(span);
    // Keep the caret BEFORE the ghost so typing flows into the real text.
  ...
  const refreshGhost = () => {
    const root = ref.current;
    if (!root || !dictRef.current) return;
    const sel = selectionOffsets(root);
    if (!sel || sel.start !== sel.end) return;
    const raw = serialize(root);
    // Don't complete mid-word (a letter immediately follows the caret).
    if (/^[\p{L}\p{M}]/u.test(raw.slice(sel.start))) return;
    const suffix = suggest(dictRef.current, raw.slice(0, sel.start));
    if (suffix) showGhost(suffix);
  };
  const acceptGhost = () => {
    const root = ref.current;
    const suffix = ghostRef.current?.textContent ?? "";
    removeGhost();
    if (!root || !suffix) return;
    const sel = selectionOffsets(root);
    const raw = serialize(root);
    const at = sel?.start ?? raw.length;
    snapshot(true);
    apply(root, raw.slice(0, at) + suffix + raw.slice(at), at + suffix.length);
  };
  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    // Tab accepts a showing ghost suggestion; every other key dismisses it.
    if (e.key === "Tab") {
      if (ghostRef.current) {
        e.preventDefault();
        acceptGhost();
    ...
    // kept after it (matches the old textarea's auto-expand).
    if (e.key === "-" && start === end) {
  ...
  return (
    <div className="relative min-h-0 flex-1">
      ...
      <div
        ref={placeholderRef}
        aria-hidden="true"
        className="pointer-events-none absolute left-2 top-2 text-havoc-muted"
